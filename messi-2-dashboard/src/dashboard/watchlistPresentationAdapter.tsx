...
function legacyHref(entry: LegacyV3Entry) {
  ...
  return enabledLegacyHref(legacyDetailHref(entry.playerId, { name: entry.snapshot.name, clubName: entry.snapshot.clubName }, dataset) ?? "") ?? datasetHref(`/players/${entry.playerId}`, dataset);
}
...
function statusBadge(label: string, current: boolean) { return <span className={current ? "rounded border border-sky-300/25 bg-sky-300/10 px-1.5 py-0.5 font-bold text-sky-100" : "rounded border border-amber-300/25 bg-amber-300/10 px-1.5 py-0.5 font-bold text-amber-100"}>{label}</span>; }
...
function legacyStatus(result: LegacyWatchlistResolution | undefined, showingCurrent: boolean, currentAvailable: boolean) {
  if (showingCurrent) return "Current server data";
  if (currentAvailable) return "Saved snapshot · current available";
...
export function legacyWatchlistPresentationRow(entry: LegacyV3Entry, result: LegacyWatchlistResolution | undefined, quality: QualityDisplay | undefined, props: PreferenceProps): LeaderboardPresentationRow<LegacyMetricKey, LegacyV3Entry> {
  ...
  const accessory = <>{contextBadge(entry)}{statusBadge(status, showingCurrent)}{showingCurrent && <DataQualityBadge quality={quality} />}{current && <span className="hidden md:inline-flex">{selector(entry, name, props.preference, props.onPreference, false)}</span>}</>;
  return {
    ...
    rank: { hidden: true, accessory: <span>{effective?.rank ?? entry.snapshot.rank ? `Rank ${effective?.rank ?? entry.snapshot.rank}` : "Rank unavailable"}</span> },
    tier: effective?.tier ?? tierFromLegacySnapshot(entry), score: effective?.score ?? entry.snapshot.score ?? null, stats: effective?.stats ?? entry.snapshot.stats ?? {},
    ...
    profileAccessory: accessory, mobileAccessory: <>{contextBadge(entry)}{statusBadge(status, showingCurrent)}{showingCurrent && <DataQualityBadge quality={quality} />}</>,
    mobileAction: current ? selector(entry, name, props.preference, props.onPreference, true) : savedOnlyAction(),
  };
}

export function duelWatchlistPresentationRow(entry: DuelPressV3Entry, result: DuelWatchlistResolution | undefined, props: PreferenceProps): LeaderboardPresentationRow<keyof DuelPressPlayerCore["stats"], DuelPressV3Entry> {
  const current = result?.status === "current" && result.player ? result.player : undefined;
  ...
  return {
    key: entry.key, source: entry, playerId: entry.playerId, identity: { kind: "full", player: effective }, identityDomId: null, detailHref: duelPressDetailHref(entry.playerId, datasetFromWatchlistV3Context(entry.context)),
    rank: { hidden: true, accessory: <span>{effective.rank ? `Rank ${effective.rank}` : "Rank unavailable"}</span> }, tier: effective.tier, score: effective.score, stats: effective.stats, minutes: effective.minutes, age: effective.age, metricSnapshot: !showingCurrent,
    profileAccessory: <>{contextBadge(entry)}{statusBadge(status, showingCurrent)}{current && <span className="hidden md:inline-flex">{selector(entry, effective.name, props.preference, props.onPreference, false)}</span>}</>,
    mobileAccessory: <>{contextBadge(entry)}{statusBadge(status, showingCurrent)}</>, mobileAction: current ? selector(entry, effective.name, props.preference, props.onPreference, true) : savedOnlyAction(),
  };
}
