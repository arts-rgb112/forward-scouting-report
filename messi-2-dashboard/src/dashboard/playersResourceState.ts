...
export type ResourceAction = { type: "clear" } | { type: "start"; requestId: number; previous?: PlayersPayload } | { type: "resolve"; requestId: number; payload: PlayersPayload } | { type: "reject"; requestId: number; error: MessiApiError };
...
export function playersResourceReducer(state: ResourceState, action: ResourceAction): ResourceState {
  if (action.type === "clear") return { type: "idle" };
