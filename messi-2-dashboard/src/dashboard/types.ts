...
export type ShotTrajectory = { schemaVersion: "shotmap-trajectory-v1"; endpointKind: "goal_mouth" | "blocked"; endX: number; endY: number; endZMeters: number | null; source: "fotmob" };
export type ShotmapPoint = { x: number; y: number; outcome: "goal" | "on_target" | "off_target" | "blocked"; xg?: number | null; xgot?: number | null; trajectory?: ShotTrajectory | null };
export type ContinuousCore =
  | { available: boolean; definitionVersion: "continuous-hdr-50-v1"; targetDensityPct: 50; achievedDensityPct: number; coreAreaPct: number; densityThreshold: number; thresholdOfPeak: number; gridColumns: 32; gridRows: 22 }
  | { available: boolean; definitionVersion: "fixed-n60-r20-v2"; targetDensityPct: 50; achievedDensityPct: number; coreAreaPct: number; densityThreshold: number; thresholdOfPeak: number; gridColumns: 32; gridRows: 22; formulaVersion: "fixed-n60-r20-v2"; ccaAreaPct: number; standardizedTarget: number; quantizationDelta: number; containedMassPct: number; validPointCount: number; lowSample: boolean };
