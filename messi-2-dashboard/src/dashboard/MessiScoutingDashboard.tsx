...
import { filterAndSortWatchlistRows, watchlistPage, watchlistRows, type WatchlistSortState } from "./watchlistViewModel";
...
export default function MessiScoutingDashboard({
  ...
  const [watchSort, setWatchSort] = useState<WatchlistSortState>({ key: "score", direction: "desc" }); const [watchPage, setWatchPage] = useState(1);
  ...
  const setWatchMinutesSort = () => { setWatchSort((current) => ({ key: "minutes", direction: current.key === "minutes" && current.direction === "desc" ? "asc" : "desc" })); setWatchPage(1); };
