...
export function WatchlistCardList({ rows, onRemove, onRetry, sort = { key: "score", direction: "desc" }, onScoreSort }: Props) {
  ...
  return <section className="space-y-2 md:hidden" aria-label="Saved player contexts"><button type="button" onClick={onScoreSort} aria-label={`Sort by M.E.S.S.I. score ${scoreOrder}`} className="min-h-11 rounded border border-white/10 bg-[#0d1112] px-3 text-xs font-bold text-zinc-300">M.E.S.S.I. score {scoreOrder === "ascending" ? "↑" : "↓"}</button>{rows.map((row) => {
    ...
    return <article key={row.key} className="rounded-lg border border-white/10 bg-[#0d1112] p-3"><div className="flex items-start justify-between gap-2"><div className="min-w-0"><a href={profileHref(row)} className="block truncate font-bold hover:text-lime-300">{profile.name}</a><p className="truncate text-[10px] text-zinc-500">{profile.position} · {profile.clubName}</p><div className="mt-2 flex flex-wrap gap-1"><span className="inline-flex rounded border border-lime-300/25 bg-lime-300/10 px-2 py-1 text-[9px] font-bold text-lime-100">{watchlistContextLabel(row.entry)}</span>{sourceBadge(row)}</div></div><div className="text-right"><b className="font-mono text-lg">{profile.score?.toFixed(1)}</b><div>{profile.tier && <TierBadge tier={profile.tier} compact />}</div></div></div><div className="mt-3 grid grid-cols-3 gap-2">{metricKeys.map((key) => <div key={key} className="rounded bg-black/20 p-2 text-center"><p className="mb-1 text-[9px] text-zinc-500">{metricConfig[key].short}</p><MetricScore playerId={row.entry.playerId} metric={key} value={profile.stats![key]!} surface="mobile" compact snapshot={snapshot} /></div>)}</div><p className="mt-3 text-[10px] text-zinc-500">{snapshot ? "Stored scores at save time; current ranks are not recomputed." : "Current server profile for this saved context."}</p><div className="mt-2 grid grid-cols-2 gap-2"><a href={profileHref(row)} className="inline-flex min-h-11 items-center justify-center rounded border border-white/10 text-xs">Detail</a><button type="button" className="min-h-11 rounded border border-white/10" onClick={() => onRemove(row.key)}>Remove this context</button></div></article>;
  })}</section>;
}
