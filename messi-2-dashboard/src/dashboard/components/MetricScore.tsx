...
export function MetricScore<K extends string = MetricKey>({ playerId, metric, value, surface, compact = false, snapshot = false, quality, presentation, metricRank }: Props<K>) {
  ...
  if (!imputed) return <div className="flex justify-center">
    ...
    {open && typeof document !== "undefined" && createPortal(<div ref={tooltipRef} id={tooltipId} role="tooltip" style={{ left: position.left, top: position.top }} className="pointer-events-none fixed z-[100] w-64 rounded-lg border border-white/10 bg-[#0a0d0f] p-3 text-left shadow-2xl"><div className="mb-1 flex items-center justify-between gap-2"><b className="text-xs text-white">{help.label}</b><span className={`rounded border px-1.5 py-0.5 type-caption font-black ${band.className}`}>{value}/100 · {rankLabel}</span></div><p className="type-caption leading-4 text-zinc-400">{help.detail}</p>{help.formula && <p className="mt-1 type-caption leading-4 text-zinc-500">평가 방식: {help.formula}</p>}</div>, document.body)}
  </div>;
