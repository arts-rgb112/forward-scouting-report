...
export function MetricScore<K extends string = MetricKey>({ playerId, metric, value, surface, compact = false, snapshot = false, quality, presentation, metricRank }: Props<K>) {
  ...
  if (!imputed) return <div className="flex justify-center">
    ...
    {open && typeof document !== "undefined" && createPortal(<div ref={tooltipRef} id={tooltipId} role="tooltip" style={{ left: position.left, top: position.top }} className="pointer-events-none fixed z-[100] w-64 rounded-lg border border-white/10 bg-[#0a0d0f] p-3 text-left shadow-2xl"><div className="mb-1 flex items-center justify-between gap-2"><b className="text-xs text-white">{help.label}</b><span className={`rounded border px-1.5 py-0.5 text-[10px] font-black ${band.className}`}>{value}/100 · {rankLabel}</span></div><p className="text-[11px] leading-4 text-zinc-400">{help.detail}</p>{help.formula && <p className="mt-1 text-[10px] leading-4 text-zinc-500">평가 방식: {help.formula}</p>}</div>, document.body)}
  </div>;
  return <div className="flex flex-col items-center justify-center gap-0.5">
    ...
    {open && typeof document !== "undefined" && createPortal(<div ref={tooltipRef} id={tooltipId} role="tooltip" style={{ left: position.left, top: position.top }} className="pointer-events-none fixed z-[100] w-64 rounded-lg border border-white/10 bg-[#0a0d0f] p-3 text-left shadow-2xl"><div className="mb-1 flex items-center justify-between gap-2"><b className="text-xs text-white">{help.label}</b><span className={`rounded border px-1.5 py-0.5 text-[10px] font-black ${band.className}`}>{value}/100 · {rankLabel}</span></div><p className="text-[11px] leading-4 text-zinc-400">{help.detail}</p>{help.formula && <p className="mt-1 text-[10px] leading-4 text-zinc-500">평가 방식: {help.formula}</p>}<p className="mt-1 text-[10px] leading-4 text-amber-100">이 능력치에는 대체값이 포함됩니다. 관측 데이터 비중: {quality?.kind === "incomplete" ? quality.dataQuality.observedWeightPct : 0}%.</p></div>, document.body)}
  </div>;
}
