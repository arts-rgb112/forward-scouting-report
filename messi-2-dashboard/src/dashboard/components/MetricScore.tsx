...
type Props = { playerId: number; metric: MetricKey; value: number; surface: "table" | "mobile"; compact?: boolean };

export function MetricScore({ playerId, metric, value, surface, compact = false }: Props) {
  ...
  return <div className="flex justify-center">
    ...
    {open && typeof document !== "undefined" && createPortal(<div ref={tooltipRef} id={tooltipId} role="tooltip" style={{ left: position.left, top: position.top }} className="pointer-events-none fixed z-[100] w-56 rounded-lg border border-white/10 bg-[#0a0d0f] p-3 text-left shadow-2xl"><div className="mb-1 flex items-center justify-between"><b className="text-xs text-white">{help.label}</b><span className="text-sm font-black text-[#aaf06b]">{value}/100</span></div><p className="text-[11px] leading-4 text-zinc-400">{help.detail}</p></div>, document.body)}
  </div>;
}
