...
export function WatchlistDrawer({ open, entries, selectedKeys, onClose, onRemove, onToggleSelection, feedback }: Props) {
  ...
  const compareHref = selectedEntries.length === 2 ? duelPressCompareHref(...selectedEntries.map((entry) => ({ playerId: entry.playerId, taxonomy: "legacy-v1" as const, context: entry.context.mode === "league" ? { season: entry.context.season, mode: "league" as const, scope: entry.context.scope ?? 8, competition: "all" as const } : { season: entry.context.season, mode: "europe" as const, scope: null, competition: entry.context.competition ?? "all" } })) as [any, any]) : null;
  return <div className="fixed inset-0 z-[80] bg-black/60" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <aside id="watchlist-drawer" role="dialog" aria-modal="true" aria-labelledby="watchlist-title" className="ml-auto flex h-full w-full max-w-md flex-col border-l border-white/10 bg-[#101415] shadow-2xl">
      <header className="flex min-h-16 items-center justify-between border-b border-white/10 px-4"><div><h2 id="watchlist-title" className="font-black">Manage / Compare watchlist</h2><p className="text-base text-zinc-500">Manage saved contexts and select two for comparison. Use Watchlist to browse them.</p></div><button ref={closeRef} type="button" onClick={onClose} className="min-h-11 min-w-11 rounded border border-white/10 text-sm" aria-label="Close watchlist manager">Close</button></header>
      <p aria-live="polite" className="min-h-8 px-4 pt-2 text-base text-lime-200">{feedback}</p>
      <div className="min-h-0 flex-1 overflow-y-auto px-4 pb-4">
        ...
        <ul className="space-y-3">{entries.map((entry) => {
          ...
          const detailHref = datasetHref(`/players/${entry.playerId}`, state);
          return <li key={entry.key} className="rounded-lg border border-white/10 bg-black/20 p-3"><div className="flex gap-3"><AssetImage src={entry.snapshot.face ?? null} alt="" kind="face" fallbackLabel={entry.snapshot.name} width={44} height={44} className="h-11 w-11 rounded object-cover" /><div className="min-w-0 flex-1"><b className="block truncate text-sm">{entry.snapshot.name}</b><p className="truncate text-base text-zinc-400">{entry.snapshot.position} · {entry.snapshot.clubName}</p><span className="mt-2 inline-flex rounded border border-lime-300/25 bg-lime-300/10 px-2 py-1 type-caption font-bold text-lime-100">{contextLabel(entry)}</span></div></div><div className="mt-3 grid grid-cols-2 gap-2"><button type="button" onClick={() => onToggleSelection(entry.key)} aria-pressed={selected} className="min-h-11 rounded border border-white/10 px-2 text-base">{selected ? "Selected for compare" : "Select for compare"}</button><button type="button" onClick={() => onRemove(entry.key)} className="min-h-11 rounded border border-white/10 px-2 text-base text-zinc-300">Remove</button><a href={detailHref} className="inline-flex min-h-11 items-center justify-center rounded border border-white/10 px-2 text-base">View detail</a><a href={leaderboardHref(state)} className="inline-flex min-h-11 items-center justify-center rounded border border-white/10 px-2 text-base">Open source leaderboard</a></div></li>;
        })}</ul>
      </div>
      <footer className="border-t border-white/10 p-4"><p className="mb-2 text-base text-zinc-500">{selectedEntries.length}/2 selected. Select two saved contexts to open the comparison page.</p>{compareHref ? <a href={compareHref} className="inline-flex min-h-11 w-full items-center justify-center rounded bg-lime-300 px-3 text-base font-black text-black">Open comparison page</a> : <span aria-disabled="true" className="inline-flex min-h-11 w-full items-center justify-center rounded bg-white/10 px-3 text-base font-black text-zinc-500">Open comparison page</span>}</footer>
    </aside>
  </div>;
}
