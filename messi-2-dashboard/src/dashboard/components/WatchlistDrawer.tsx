    ...
    <aside id="watchlist-drawer" role="dialog" aria-modal="true" aria-labelledby="watchlist-title" className="ml-auto flex h-full w-full max-w-md flex-col border-l border-white/10 bg-[#101415] shadow-2xl">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-4 pb-4">
        ...
        <ul className="space-y-3">{entries.map((entry) => {
          ...
          return <li key={entry.key} className="rounded-lg border border-white/10 bg-black/20 p-3"><div className="flex gap-3"><AssetImage src={entry.snapshot.face ?? null} alt="" kind="face" fallbackLabel={entry.snapshot.name} width={44} height={44} className="h-11 w-11 rounded object-cover" /><div className="min-w-0 flex-1"><b className="block truncate text-sm">{entry.snapshot.name}</b><p className="truncate text-xs text-zinc-400">{entry.snapshot.position} · {entry.snapshot.clubName}</p><span className="mt-2 inline-flex rounded border border-lime-300/25 bg-lime-300/10 px-2 py-1 type-caption font-bold text-lime-100">{contextLabel(entry)}</span></div></div><div className="mt-3 grid grid-cols-2 gap-2"><button type="button" onClick={() => onToggleSelection(entry.key)} aria-pressed={selected} className="min-h-11 rounded border border-white/10 px-2 text-xs">{selected ? "Selected for compare" : "Select for compare"}</button><button type="button" onClick={() => onRemove(entry.key)} className="min-h-11 rounded border border-white/10 px-2 text-xs text-zinc-300">Remove</button><a href={detailHref} className="inline-flex min-h-11 items-center justify-center rounded border border-white/10 px-2 text-xs">View detail</a><a href={leaderboardHref(state)} className="inline-flex min-h-11 items-center justify-center rounded border border-white/10 px-2 text-xs">Open source leaderboard</a></div></li>;
        })}</ul>
      </div>
