...
export function WatchlistDrawer({ open, entries, selectedKeys, onClose, onRemove, onToggleSelection, feedback }: Props) {
  ...
  return <div className="fixed inset-0 z-[80] bg-black/60" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <aside id="watchlist-drawer" role="dialog" aria-modal="true" aria-labelledby="watchlist-title" className="ml-auto flex h-full w-full max-w-md flex-col border-l border-white/10 bg-[#101415] shadow-2xl">
      <header className="flex min-h-16 items-center justify-between border-b border-white/10 px-4"><div><h2 id="watchlist-title" className="font-black">Watchlist</h2><p className="text-xs text-zinc-500">Saved scouting contexts, not a leaderboard filter.</p></div><button ref={closeRef} type="button" onClick={onClose} className="min-h-11 min-w-11 rounded border border-white/10 text-sm" aria-label="Close watchlist">Close</button></header>
      ...
      <footer className="border-t border-white/10 p-4"><p className="mb-2 text-xs text-zinc-500">{selectedKeys.length}/2 selected. Selections are not prefilled on the legacy comparison page.</p><a href={compareHref} className="inline-flex min-h-11 w-full items-center justify-center rounded bg-lime-300 px-3 text-xs font-black text-black">Open comparison page</a></footer>
    </aside>
  </div>;
}
