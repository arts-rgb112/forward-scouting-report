...
export function LeaderboardPagination({ page, total, pageSize, pending = false, onPageChange }: Props) {
  ...
  const button = "min-h-11 min-w-11 rounded-md border border-white/10 px-3 text-base font-bold text-zinc-300 disabled:cursor-not-allowed disabled:opacity-35 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-lime-300";
  return <nav aria-label="Leaderboard pagination" aria-busy={pending} className="mt-4 flex flex-wrap items-center justify-between gap-3 rounded-lg border border-white/10 bg-[#101415] p-2">
    ...
    <span className="shrink-0 whitespace-nowrap px-1 text-base text-zinc-500">Page {page} of {pages}</span>
  </nav>;
}
