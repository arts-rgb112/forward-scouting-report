...
import type { MetricRanksByTargetKey } from "../useMetricRanks";

type RemoveResult = boolean | void | Promise<boolean | void>;
type Props = { entries: readonly WatchlistV3Entry[]; resolutions: Readonly<Record<string, DuelWatchlistResolution>>; legacyResolutions?: Readonly<Record<string, LegacyWatchlistResolution>>; legacyQuality?: Readonly<Record<string, QualityDisplay>>; metricRanks?: MetricRanksByTargetKey; preferences: Readonly<Record<string, "saved" | "current">>; fallbackFocusRef: RefObject<HTMLElement | null>; sort: WatchlistV3CommonSort; direction: "asc" | "desc"; onMetricSort(key: WatchlistV3CommonSort): void; onPreference(key: string, value: "saved" | "current"): void; onRemove(key: string): RemoveResult; onRetry(): void };
...
export function TaxonomyWatchlistView(props: Props) {
  ...
  const watch = <E extends WatchlistV3Entry>(): WatchPresentation<E> => ({
    ...
    accessibleLabel: (entry) => `Remove ${entry.snapshot.name} ${watchlistV3ContextLabel(entry)} from watchlist`, visibleLabel: (_entry, watched, surface) => surface === "table" ? (watched ? "✓" : "+") : (watched ? "✓ Saved" : "Watch"),
  ...
  const sort = { key: props.sort, direction: props.direction };
  const legacySort = (key: "score" | typeof legacyMetricKeys[number]) => props.onMetricSort(key);
  const duelSort = (key: "score" | typeof DUEL_PRESS_METRIC_KEYS[number]) => props.onMetricSort(key);

  return <div className="space-y-8">
    {legacy.length > 0 && <section aria-labelledby="legacy-watchlist-heading" className="space-y-2"><div><h2 ref={legacyHeading} tabIndex={-1} id="legacy-watchlist-heading" className="text-sm font-black uppercase tracking-wider">Legacy M.E.S.S.I. taxonomy</h2><p className="text-base text-zinc-500">Original Aerial and Ground duel metrics are preserved.</p></div><LeaderboardPlayerCardList players={legacy} metricKeys={legacyMetricKeys} metricRegistry={metricConfig} sort={sort} onMetricSort={legacySort} rowAdapter={(entry) => legacyWatchlistPresentationRow(entry, props.legacyResolutions?.[entry.key], props.legacyQuality?.[entry.key], { preference: props.preferences[entry.key], onPreference: props.onPreference })} caption="Saved legacy taxonomy contexts" density="watchlist" watch={{ ...legacyWatch, onToggle: (entry) => remove(entry, "mobile") }} /><LeaderboardPlayerTable players={legacy} metricKeys={legacyMetricKeys} metricRegistry={metricConfig} sort={sort} onMetricSort={legacySort} onMinutesSort={() => props.onMetricSort("minutes")} rowAdapter={(entry) => legacyWatchlistPresentationRow(entry, props.legacyResolutions?.[entry.key], props.legacyQuality?.[entry.key], { preference: props.preferences[entry.key], onPreference: props.onPreference })} caption="Saved legacy taxonomy contexts" density="watchlist" watch={legacyWatch} /></section>}
    {duel.length > 0 && <section aria-labelledby="duel-watchlist-heading" className="space-y-2"><div className="flex flex-wrap items-center justify-between gap-2"><div><h2 ref={duelHeading} tabIndex={-1} id="duel-watchlist-heading" className="text-sm font-black uppercase tracking-wider">Duel / Press taxonomy</h2><p className="text-base text-zinc-500">Combined Duel and Forward Press use duel-press-v1 only.</p></div><button type="button" onClick={props.onRetry} className="min-h-11 rounded border border-white/10 px-3 text-base">Refresh visible contexts</button></div><LeaderboardPlayerCardList players={duel} metricKeys={DUEL_PRESS_METRIC_KEYS} metricRegistry={duelPressMetricConfig} sort={sort} onMetricSort={duelSort} rowAdapter={(entry) => duelWatchlistPresentationRow(entry, props.resolutions[entry.key], { preference: props.preferences[entry.key], onPreference: props.onPreference }, props.metricRanks?.[entry.key])} caption="Saved duel and press taxonomy contexts" density="watchlist" watch={{ ...duelWatch, onToggle: (entry) => remove(entry, "mobile") }} /><LeaderboardPlayerTable players={duel} metricKeys={DUEL_PRESS_METRIC_KEYS} metricRegistry={duelPressMetricConfig} sort={sort} onMetricSort={duelSort} onMinutesSort={() => props.onMetricSort("minutes")} rowAdapter={(entry) => duelWatchlistPresentationRow(entry, props.resolutions[entry.key], { preference: props.preferences[entry.key], onPreference: props.onPreference }, props.metricRanks?.[entry.key])} caption="Saved duel and press taxonomy contexts" density="watchlist" watch={duelWatch} /></section>}
  </div>;
}
