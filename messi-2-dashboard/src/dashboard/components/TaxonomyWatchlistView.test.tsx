...
function View({ entries, resolutions = {}, legacyResolutions = {}, metricRanks, onRemove = noop }: { entries: readonly WatchlistV3Entry[]; resolutions?: React.ComponentProps<typeof TaxonomyWatchlistView>["resolutions"]; legacyResolutions?: NonNullable<React.ComponentProps<typeof TaxonomyWatchlistView>["legacyResolutions"]>; metricRanks?: React.ComponentProps<typeof TaxonomyWatchlistView>["metricRanks"]; onRemove?: React.ComponentProps<typeof TaxonomyWatchlistView>["onRemove"] }) {
  ...
  return <><p ref={fallbackFocusRef} tabIndex={-1}>Watchlist results summary · {entries.length} saved contexts</p><TaxonomyWatchlistView entries={entries} resolutions={resolutions} legacyResolutions={legacyResolutions} metricRanks={metricRanks} preferences={preferences} fallbackFocusRef={fallbackFocusRef} onPreference={(key, value) => setPreferences((state) => ({ ...state, [key]: value }))} onRemove={onRemove} onRetry={noop} /></>;
}
...
describe("shared Watchlist V3 leaderboard presentation", () => {
  it("renders the duel taxonomy as an exact 12-column shared table with bands, tier, and snapshot tooltips", async () => {
    ...
    expect(await screen.findByRole("tooltip")).toHaveTextContent("저장 시점 점수"); expect(screen.getAllByText("Offline · saved snapshot")).toHaveLength(2);
    expect(screen.getAllByRole("button", { name: /Remove Harry Kane.*2025\/2026.*8 leagues/i })).toHaveLength(2); expect(screen.getAllByText("✓")).toHaveLength(1);
  });
  ...
  it("keeps same-player contexts independently keyed, omits duplicate DOM ids, preserves exact hrefs, and removes the exact key", () => {
    ...
    const hrefs = new Set(screen.getAllByRole("link", { name: player.name }).map((link) => link.getAttribute("href"))); if (import.meta.env.VITE_LEGACY_DETAIL_HANDOFF_ENABLED === "true") { expect([...hrefs].every((href) => href?.includes("streamlit.app"))).toBe(true); expect([...hrefs].some((href) => href?.includes("season=24%2F25"))).toBe(true); } else expect(hrefs).toEqual(new Set([duelPressDetailHref(player.id, context), duelPressDetailHref(player.id, otherContext)]));
  ...
  it("switches every displayed field atomically between current and saved duel profiles without changing the saved-context href", async () => {
    ...
    render(<View entries={[entry]} resolutions={{ [entry.key]: { status: "current", player: current } }} metricRanks={{ [entry.key]: { outsideShot: { state: "resolved", rank: 14, population: 673 } } }} />);
    expect(screen.getAllByRole("link", { name: current.name })).toHaveLength(2); expect(screen.getAllByRole("link", { name: current.name })[0]).toHaveAttribute("href", import.meta.env.VITE_LEGACY_DETAIL_HANDOFF_ENABLED === "true" ? expect.stringContaining("streamlit.app") : duelPressDetailHref(entry.playerId, context)); expect(screen.getAllByRole("img", { name: `${current.name} portrait` })[0]).toHaveAttribute("src", current.face); expect(screen.getByText("Current Club · Current League")).toBeInTheDocument(); expect(screen.getAllByText("Current Position")).toHaveLength(2); expect(screen.getAllByText("Type B")).toHaveLength(2); expect(screen.getAllByText("현재 서버 순위 7")).toHaveLength(2); expect(screen.getAllByText("77.7")).toHaveLength(2); expect(screen.getAllByText("1,111").length).toBeGreaterThan(0); expect(screen.getAllByText("21").length).toBeGreaterThan(0); expect(screen.getAllByLabelText("Overall M.E.S.S.I. tier: Emerald, level 2")).toHaveLength(2); expect(screen.getAllByLabelText(new RegExp(`^${duelPressMetricConfig.outsideShot.label} 11.1`))).toHaveLength(2);
    fireEvent.focus(screen.getAllByLabelText(new RegExp(`^${duelPressMetricConfig.outsideShot.label} 11.1`))[0]); expect(await screen.findByRole("tooltip")).toHaveTextContent("전체 14위 / 673명"); fireEvent.blur(screen.getAllByLabelText(new RegExp(`^${duelPressMetricConfig.outsideShot.label} 11.1`))[0]);
    fireEvent.change(screen.getAllByRole("combobox", { name: `${current.name} saved or current snapshot` })[0], { target: { value: "saved" } });
    expect(screen.queryByRole("link", { name: current.name })).not.toBeInTheDocument(); expect(screen.getAllByRole("link", { name: player.name })).toHaveLength(2); expect(screen.getAllByRole("img", { name: `${player.name} portrait` })[0]).toHaveAttribute("src", player.face); expect(screen.getByText(`${player.club.name} · ${player.league.name}`)).toBeInTheDocument(); expect(screen.getAllByText(`저장 시점 순위 ${player.rank}`)).toHaveLength(2); expect(screen.getAllByText(player.score.toFixed(1))).toHaveLength(2); expect(screen.getAllByLabelText(new RegExp(`^${duelPressMetricConfig.outsideShot.label} ${player.stats.outsideShot}`))).toHaveLength(2); expect(screen.getAllByRole("link", { name: player.name })[0]).toHaveAttribute("href", import.meta.env.VITE_LEGACY_DETAIL_HANDOFF_ENABLED === "true" ? expect.stringContaining("streamlit.app") : duelPressDetailHref(entry.playerId, context));
    expect(screen.queryByText("Saved snapshot · current available")).not.toBeInTheDocument();
    fireEvent.focus(screen.getAllByLabelText(new RegExp(`^${duelPressMetricConfig.outsideShot.label} ${player.stats.outsideShot}`))[0]); expect(await screen.findByRole("tooltip")).toHaveTextContent("전체 순위 정보 없음");
  });
  ...
  it("keeps legacy current/saved identity and quality atomic", () => {
    ...
    const { container } = render(<LegacyView />); expect(screen.getAllByRole("link", { name: current.name })).toHaveLength(2); expect(screen.getAllByText("현재 서버 순위 19")).toHaveLength(2); expect(screen.getAllByLabelText(new RegExp(`^${metricConfig.outsideShot.label} 14.2`))).toHaveLength(2); expect(container.querySelectorAll('[aria-describedby^="quality-tooltip-"]')).toHaveLength(2);
    fireEvent.change(screen.getAllByRole("combobox", { name: `${current.name} saved or current snapshot` })[0], { target: { value: "saved" } }); expect(screen.queryByRole("link", { name: current.name })).not.toBeInTheDocument(); expect(screen.getAllByRole("link", { name: snapshot.name })).toHaveLength(2); expect(screen.getAllByText(`저장 시점 순위 ${snapshot.rank}`)).toHaveLength(2); expect(container.querySelectorAll('[aria-describedby^="quality-tooltip-"]')).toHaveLength(0); expect(screen.queryByText("Saved snapshot · current available")).not.toBeInTheDocument();
  });
  ...
  it("renders a historical partial legacy snapshot without fabricated assets, tier, rank, or metrics", () => {
    ...
    expect(screen.queryByRole("img")).not.toBeInTheDocument(); expect(screen.getAllByLabelText("Historical Player portrait unavailable")).toHaveLength(2); expect(screen.getAllByLabelText("Tier unavailable")).toHaveLength(2); expect(screen.getAllByText("저장 시점 순위 정보 없음")).toHaveLength(2);
