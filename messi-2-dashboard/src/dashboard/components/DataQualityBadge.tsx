...
export function DataQualityBadge({ quality, placement = "score" }: Props) {
  ...
  return <><span ref={triggerRef} tabIndex={0} aria-describedby={id} aria-label={label} onMouseEnter={() => setOpen(true)} onMouseLeave={() => setOpen(false)} onFocus={() => setOpen(true)} onBlur={() => setOpen(false)} className={`inline-flex cursor-help rounded border px-1 py-0.5 type-caption font-bold leading-none outline-none focus-visible:ring-2 focus-visible:ring-[#8cff68] ${placement === "metric" ? "ml-1" : "ml-2"} ${isIncomplete ? "border-amber-300/35 bg-amber-300/10 text-amber-100" : "border-zinc-300/35 bg-zinc-300/10 text-zinc-200"}`}>{label}</span>
    {open && typeof document !== "undefined" && createPortal(<div ref={tooltipRef} id={id} role="tooltip" style={{ left: position.left, top: position.top }} className="pointer-events-none fixed z-[100] w-60 rounded-lg border border-white/10 bg-[#0a0d0f] p-3 text-left type-caption leading-4 text-zinc-300 shadow-2xl">{detail}</div>, document.body)}
  </>;
}
