...
export function WatchlistV3SortControls({ sort, direction, onChange }: { sort: WatchlistV3CommonSort; direction: "asc" | "desc"; onChange(sort: WatchlistV3CommonSort, direction: "asc" | "desc"): void }) {
  return <div className="mb-3 flex min-h-11 flex-wrap items-center gap-2" aria-label="Saved context sorting">
    <label className="text-base text-zinc-400">저장된 컨텍스트 정렬
      <select aria-label="Watchlist 정렬 기준" value={sort} onChange={(event) => onChange(event.target.value as WatchlistV3CommonSort, direction)} className="ml-2 min-h-11 rounded border border-white/10 bg-[#111516] px-3 text-base text-zinc-200">
    ...
    <button type="button" aria-label={`Sort direction ${direction === "asc" ? "ascending" : "descending"}`} onClick={() => onChange(sort, direction === "asc" ? "desc" : "asc")} className="min-h-11 rounded border border-white/10 px-3 text-base">{direction === "asc" ? "오름차순 ↑" : "내림차순 ↓"}</button>
