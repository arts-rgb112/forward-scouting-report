...
export function PlayerIdentity({ player, partial = false, mobile = false, dataset, detailHref: suppliedDetailHref, domId }: PlayerIdentityProps) {
  ...
  const detailHref = suppliedDetailHref ?? enabledLegacyHref(legacyDetailHref(player.id, { name: player.name, clubName: clubName ?? "" }, current) ?? "") ?? datasetHref(`/players/${player.id}`, current);
