...
export function PlayerIdentity({ player, partial = false, mobile = false, dataset, detailHref: suppliedDetailHref, domId }: PlayerIdentityProps) {
  ...
  return <div id={resolvedDomId} className="flex min-w-0 items-center gap-3">
    ...
    <div className="min-w-0"><a href={detailHref} className="block truncate type-label font-extrabold hover:text-lime-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-lime-300">{player.name}</a>
      {mobile && <p className="mt-0.5 truncate type-caption text-zinc-500">{clubName ?? "—"} · {leagueName ?? "—"}</p>}
      <div className="mt-1 flex items-center gap-1.5">{player.nation && <AssetImage src={player.nation.icon} alt="" kind="nation" fallbackLabel={player.nation.name} width={20} height={16} className="h-4 w-5 rounded-sm object-cover" />}{player.league && <AssetImage src={player.league.icon} alt="" kind="league" fallbackLabel={leagueFallbackLabel(player.league)} width={16} height={16} className="h-4 w-4 rounded-sm object-cover" />}{player.club && <AssetImage src={player.club.icon} alt="" kind="club" fallbackLabel={player.club.name} width={16} height={16} className="h-4 w-4 rounded-sm object-cover" />}{player.position ? <span className="ml-1 rounded border border-white/10 px-1.5 py-0.5 font-mono type-caption text-zinc-400">{player.position}</span> : <span aria-label="Position unavailable" className="ml-1 type-caption text-zinc-600">—</span>}{player.archetype && <span className="type-caption text-zinc-600">{player.archetype}</span>}</div>
    </div>
  </div>;
}
