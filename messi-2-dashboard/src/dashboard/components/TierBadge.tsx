...
export function TierBadge({ tier, compact: _compact = false }: { tier: Tier; compact?: boolean }) {
  ...
  return <span aria-label={`Overall M.E.S.S.I. tier: ${presentation.label}, level ${tier.level}`} className={`inline-flex max-w-full items-center justify-center gap-1 rounded border px-1.5 py-1 font-mono type-caption font-bold leading-tight ${presentation.className}`} title={presentation.tooltip}>
