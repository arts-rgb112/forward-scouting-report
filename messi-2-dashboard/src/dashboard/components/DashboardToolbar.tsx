...
export function DashboardToolbar(props: Props) {
  ...
  const queryRef = useRef(props.query); queryRef.current = props.query;
  const queryNamespace = props.viewMode ?? "leaderboard";
  const queryNamespaceRef = useRef(queryNamespace);
  const hadFiltersRef = useRef(props.hasFilters);
  const committedQueryRef = useRef(props.query);
  ...
  const scheduleCommit = (value: string) => {
    cancelCommit();
    if (!value) { commitNow(""); return; }
    if (value === queryRef.current) return;
    debounceTimerRef.current = window.setTimeout(() => {
      debounceTimerRef.current = undefined;
      if (!composingRef.current && value !== queryRef.current) onQueryChangeRef.current(value);
    }, 350);
  };
  useEffect(() => {
    const queryChanged = committedQueryRef.current !== props.query;
    committedQueryRef.current = props.query;
    if (queryNamespaceRef.current !== queryNamespace) {
      queryNamespaceRef.current = queryNamespace;
      hadFiltersRef.current = props.hasFilters;
      composingRef.current = false;
      cancelCommit();
      if (needle !== props.query) setNeedle(props.query);
      setOpen(false);
      setActive(-1);
      return;
    }
    if (hadFiltersRef.current && !props.hasFilters) {
      hadFiltersRef.current = props.hasFilters;
      composingRef.current = false;
      cancelCommit();
      if (needle !== props.query) setNeedle(props.query);
    ...
    if (composingRef.current || props.query === needle) return;
    cancelCommit();
    setNeedle(props.query);
  ...
  const onKeyDown = (event: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    else if (event.key === "Enter") { event.preventDefault(); commitNow(needle); }
