...
function WatchButton<K extends string, P>({ watch, row, surface }: { watch: WatchPresentation<P>; row: LeaderboardPresentationRow<K, P>; surface: "table" | "mobile" }) {
  ...
  return <button ref={(element) => watch.buttonRef?.(row.source, surface, element)} disabled={!watch.available} title={!watch.available ? unavailable : undefined} className={surface === "table" ? (watch.available ? "min-h-11 min-w-11 rounded focus-visible:ring-2 focus-visible:ring-lime-300" : "min-h-11 min-w-11 rounded type-caption text-zinc-500 disabled:cursor-not-allowed") : (watch.available ? "min-h-11 w-full rounded border border-white/10" : "min-h-11 w-full rounded border border-white/10 text-zinc-500 disabled:cursor-not-allowed")} onClick={watch.available ? () => { void watch.onToggle(row.source); } : undefined} aria-pressed={watch.available ? watched : false} aria-label={watch.accessibleLabel?.(row.source) ?? `${row.identity.player.name} watchlist${watch.available ? "" : `, ${unavailable}`}`}>{visible}</button>;
}
...
export function LeaderboardPlayerTable<K extends string, P>(props: SharedProps<K, P>) {
  ...
  return <section className="hidden rounded-lg border border-white/10 bg-[#0d1112] md:block"><div className="overflow-x-auto"><table className="w-full min-w-[1340px] table-fixed border-collapse"><caption className="sr-only">{props.caption ?? "Scouting dataset players and six sector scores"}</caption><LeaderboardTableColumns metricKeys={props.metricKeys} density={props.density} /><LeaderboardTableHeader metricKeys={props.metricKeys} metricRegistry={props.metricRegistry} sort={props.sort} onMetricSort={props.onMetricSort} onMinutesSort={props.onMinutesSort} /><tbody>{rows.map((row) => <tr key={row.key} className={`group ${props.density === "watchlist" ? "h-20" : "h-[72px]"} border-b border-white/[.07] hover:bg-lime-300/[.04]`}><th scope="row" className="sticky left-0 z-10 bg-[#0d1112] px-3 text-left group-hover:bg-[#121916]"><div className="flex items-center gap-3">{!row.rank.hidden && <span className="w-6 font-mono type-caption text-zinc-600">{row.rank.prefix ?? "—"}</span>}<div className="min-w-0 flex-1"><Identity row={row} dataset={props.dataset} />{(row.profileAccessory || row.rank.accessory) && <div className="ml-[60px] mt-1 flex flex-wrap items-center gap-1.5 type-caption text-zinc-400">{row.rank.accessory}{row.profileAccessory}</div>}</div></div></th><td className="px-1 text-center"><div className="flex flex-col items-center gap-1">{row.tier ? <TierBadge tier={row.tier} compact /> : unavailableValue("Tier")}{row.tierAccessory && <span className="type-caption text-zinc-500">{row.tierAccessory}</span>}</div></td><td title={row.scorePercentile === undefined ? undefined : `Cohort percentile: ${row.scorePercentile}`} className="text-center font-mono text-lg font-black">{row.score === null ? unavailableValue("M.E.S.S.I. score") : row.score.toFixed(1)}</td>{props.metricKeys.map((key) => <td key={key}>{row.stats[key] === undefined ? unavailableValue(props.metricRegistry[key].label) : <MetricScore playerId={row.playerId} metric={key} value={row.stats[key]} surface="table" snapshot={row.metricSnapshot} quality={row.metricQuality} presentation={props.metricRegistry[key]} metricRank={rankFor(row, key)} />}</td>)}<td className="px-2 text-right font-mono text-base">{row.minutes === null ? unavailableValue("Minutes") : row.minutes.toLocaleString()}</td><td title={row.age === null ? "Birth date unavailable" : undefined} className="text-center font-mono text-base">{row.age ?? "—"}</td><td className="text-center"><WatchButton watch={props.watch} row={row as LeaderboardPresentationRow<string, P>} surface="table" /></td></tr>)}</tbody></table></div></section>;
}
...
export function LeaderboardPlayerCardList<K extends string, P>(props: SharedProps<K, P>) {
  ...
  const rankFor = (row: LeaderboardPresentationRow<K, P>, key: K) => row.metricRanks?.[key] ?? props.metricRanksByPlayerId?.[row.playerId]?.[key];
  return <section className="space-y-2 md:hidden" aria-label="Mobile player list">{props.onMetricSort && <button type="button" onClick={() => props.onMetricSort?.("score")} aria-label={`Sort by M.E.S.S.I. score ${scoreOrder}`} className="min-h-11 rounded border border-white/10 bg-[#0d1112] px-3 text-base font-bold text-zinc-300">M.E.S.S.I. score {scoreOrder === "ascending" ? "↑" : "↓"}</button>}{rows.map((row) => <article key={row.key} className="rounded-lg border border-white/10 bg-[#0d1112] p-3"><div className="flex items-start gap-2">{!row.rank.hidden && <span className="pt-1 font-mono type-caption text-zinc-600">{row.rank.mobilePrefix ?? row.rank.prefix ?? "—"}</span>}<div className="min-w-0 flex-1"><div className="flex justify-between gap-2"><Identity row={row} dataset={props.dataset} mobile /><div className="text-right">{row.score === null ? unavailableValue("M.E.S.S.I. score") : <b title={row.scorePercentile === undefined ? undefined : `Cohort percentile: ${row.scorePercentile}`} className="font-mono text-lg">{row.score.toFixed(1)}</b>}<div>{row.tier ? <TierBadge tier={row.tier} compact /> : unavailableValue("Tier")}</div>{row.tierAccessory && <span className="type-caption text-zinc-500">{row.tierAccessory}</span>}</div></div></div></div>{(row.mobileAccessory || row.profileAccessory || row.rank.accessory) && <div className="mt-2 flex flex-wrap items-center gap-1.5 type-caption text-zinc-400">{row.rank.accessory}{row.mobileAccessory ?? row.profileAccessory}</div>}<div className="mt-3 grid grid-cols-3 gap-2">{props.metricKeys.map((key) => <div key={key} className="rounded bg-black/20 p-2 text-center"><p className="mb-1 type-caption text-zinc-500">{props.metricRegistry[key].short}</p>{row.stats[key] === undefined ? unavailableValue(props.metricRegistry[key].label) : <MetricScore playerId={row.playerId} metric={key} value={row.stats[key]} surface="mobile" compact snapshot={row.metricSnapshot} quality={row.metricQuality} presentation={props.metricRegistry[key]} metricRank={rankFor(row, key)} />}</div>)}</div><div className={`mt-3 ${row.mobileAction ? "grid grid-cols-2 gap-2" : ""}`}>{row.mobileAction}<WatchButton watch={props.watch} row={row as LeaderboardPresentationRow<string, P>} surface="mobile" /></div></article>)}</section>;
}
