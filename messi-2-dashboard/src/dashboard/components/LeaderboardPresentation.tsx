...
export type LeaderboardPresentationPlayer<K extends string> = PlayerIdentityData & {
  rank: number; age: number | null; minutes: number; tier: Tier; score: number; stats: Record<K, number>;
};
...
export type LeaderboardPresentationRow<K extends string, P> = {
  ...
  rank: RankPresentation; tier: Tier | null; score: number | null; stats: Partial<Record<K, number>>;
...
type SharedProps<K extends string, P> = {
  ...
  sort: PresentationSort; onMetricSort?(key: "score" | K): void; detailHref?(player: P): string; watch: WatchPresentation<P>;
...
function defaultRow<K extends string, P>(player: P, detailHref?: (player: P) => string): LeaderboardPresentationRow<K, P> {
  ...
  return { key: String(player.id), source: player, playerId: player.id, identity: { kind: "full", player }, detailHref: detailHref?.(player), rank: { prefix: String(player.rank).padStart(2, "0"), mobilePrefix: player.rank }, tier: player.tier, score: player.score, stats: player.stats, minutes: player.minutes, age: player.age };
}
...
export function LeaderboardPlayerTable<K extends string, P>(props: SharedProps<K, P>) {
  ...
  return <section className="hidden rounded-lg border border-white/10 bg-[#0d1112] md:block"><div className="overflow-x-auto"><table className="w-full min-w-[1340px] table-fixed border-collapse"><caption className="sr-only">{props.caption ?? "Scouting dataset players and six sector scores"}</caption><LeaderboardTableColumns metricKeys={props.metricKeys} density={props.density} /><LeaderboardTableHeader metricKeys={props.metricKeys} metricRegistry={props.metricRegistry} sort={props.sort} onMetricSort={props.onMetricSort} /><tbody>{rows.map((row) => <tr key={row.key} className={`group ${props.density === "watchlist" ? "h-20" : "h-[72px]"} border-b border-white/[.07] hover:bg-lime-300/[.04]`}><th scope="row" className="sticky left-0 z-10 bg-[#0d1112] px-3 text-left group-hover:bg-[#121916]"><div className="flex items-center gap-3">{!row.rank.hidden && <span className="w-6 font-mono text-[10px] text-zinc-600">{row.rank.prefix ?? "—"}</span>}<div className="min-w-0 flex-1"><Identity row={row} dataset={props.dataset} />{(row.profileAccessory || row.rank.accessory) && <div className="ml-[60px] mt-1 flex flex-wrap items-center gap-1.5 text-[9px] text-zinc-400">{row.rank.accessory}{row.profileAccessory}</div>}</div></div></th><td className="px-1 text-center">{row.tier ? <TierBadge tier={row.tier} compact /> : unavailableValue("Tier")}</td><td className="text-center font-mono text-lg font-black">{row.score === null ? unavailableValue("M.E.S.S.I. score") : row.score.toFixed(1)}</td>{props.metricKeys.map((key) => <td key={key}>{row.stats[key] === undefined ? unavailableValue(props.metricRegistry[key].label) : <MetricScore playerId={row.playerId} metric={key} value={row.stats[key]} surface="table" snapshot={row.metricSnapshot} quality={row.metricQuality} presentation={props.metricRegistry[key]} />}</td>)}<td className="px-2 text-right font-mono text-xs">{row.minutes === null ? unavailableValue("Minutes") : row.minutes.toLocaleString()}</td><td title={row.age === null ? "Birth date unavailable" : undefined} className="text-center font-mono text-xs">{row.age ?? "—"}</td><td className="text-center"><WatchButton watch={props.watch} row={row as LeaderboardPresentationRow<string, P>} surface="table" /></td></tr>)}</tbody></table></div></section>;
}
...
export function LeaderboardPlayerCardList<K extends string, P>(props: SharedProps<K, P>) {
  ...
  return <section className="space-y-2 md:hidden" aria-label="Mobile player list">{props.onMetricSort && <button type="button" onClick={() => props.onMetricSort?.("score")} aria-label={`Sort by M.E.S.S.I. score ${scoreOrder}`} className="min-h-11 rounded border border-white/10 bg-[#0d1112] px-3 text-xs font-bold text-zinc-300">M.E.S.S.I. score {scoreOrder === "ascending" ? "↑" : "↓"}</button>}{rows.map((row) => <article key={row.key} className="rounded-lg border border-white/10 bg-[#0d1112] p-3"><div className="flex items-start gap-2">{!row.rank.hidden && <span className="pt-1 font-mono text-[10px] text-zinc-600">{row.rank.mobilePrefix ?? row.rank.prefix ?? "—"}</span>}<div className="min-w-0 flex-1"><div className="flex justify-between gap-2"><Identity row={row} dataset={props.dataset} mobile /><div className="text-right">{row.score === null ? unavailableValue("M.E.S.S.I. score") : <b className="font-mono text-lg">{row.score.toFixed(1)}</b>}<div>{row.tier ? <TierBadge tier={row.tier} compact /> : unavailableValue("Tier")}</div></div></div></div></div>{(row.mobileAccessory || row.profileAccessory || row.rank.accessory) && <div className="mt-2 flex flex-wrap items-center gap-1.5 text-[10px] text-zinc-400">{row.rank.accessory}{row.mobileAccessory ?? row.profileAccessory}</div>}<div className="mt-3 grid grid-cols-3 gap-2">{props.metricKeys.map((key) => <div key={key} className="rounded bg-black/20 p-2 text-center"><p className="mb-1 text-[9px] text-zinc-500">{props.metricRegistry[key].short}</p>{row.stats[key] === undefined ? unavailableValue(props.metricRegistry[key].label) : <MetricScore playerId={row.playerId} metric={key} value={row.stats[key]} surface="mobile" compact snapshot={row.metricSnapshot} quality={row.metricQuality} presentation={props.metricRegistry[key]} />}</div>)}</div><div className={`mt-3 ${row.mobileAction ? "grid grid-cols-2 gap-2" : ""}`}>{row.mobileAction}<WatchButton watch={props.watch} row={row as LeaderboardPresentationRow<string, P>} surface="mobile" /></div></article>)}</section>;
}
