...
export function WatchlistV3Drawer({ open, entries, selectedKeys, feedback, onClose, onRemove, onToggleSelection }: Props) {
  ...
  const compareHref = selectedEntries.length === 2 ? duelPressCompareHref(...selectedEntries.map((entry) => ({ playerId: entry.playerId, taxonomy: entry.taxonomy, context: entry.context })) as [any, any]) : null;
  return <div className="fixed inset-0 z-[80] bg-black/70" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><aside ref={panel} role="dialog" aria-modal="true" aria-labelledby="watchlist-v3-title" className="ml-auto flex h-full w-full max-w-xl flex-col bg-[#0d1112] text-zinc-100 shadow-2xl"><header className="flex items-center justify-between border-b border-white/10 p-4"><div><h2 id="watchlist-v3-title" className="font-black">Manage saved contexts</h2><p className="text-base text-zinc-500">{entries.length} exact contexts · {selectedKeys.length}/2 selected</p></div><button ref={close} type="button" onClick={onClose} aria-label="Close watchlist manager" className="min-h-11 min-w-11 rounded border border-white/10">×</button></header><p role="status" aria-live="polite" className="min-h-8 px-4 py-2 text-base text-zinc-400">{feedback}</p><div className="flex-1 overflow-y-auto p-4"><ul className="space-y-2">{entries.map((entry) => { const selected = selectedKeys.includes(entry.key); return <li key={entry.key} className="rounded border border-white/10 p-3"><div className="flex items-start justify-between gap-2"><div><b className="text-sm">{entry.snapshot.name}</b><p className="text-base text-zinc-500">{entry.taxonomy === "duel-press-v1" ? "Duel / Press" : "Legacy"} · {context(entry)}</p><p className="type-caption text-zinc-600">Saved {new Date(entry.savedAt).toLocaleString()}</p></div>{selected && <span className="rounded bg-lime-300/10 px-2 py-1 type-caption text-lime-200">Position {selectedKeys.indexOf(entry.key) + 1}</span>}</div><div className="mt-3 grid grid-cols-3 gap-2"><a href={detailHref(entry)} className="inline-flex min-h-11 items-center justify-center rounded border border-white/10 px-2 text-base">View detail</a><button type="button" aria-pressed={selected} onClick={() => onToggleSelection(entry.key)} className="min-h-11 rounded border border-white/10 px-2 text-base">{selected ? "Selected for compare" : "Select for compare"}</button><button type="button" onClick={() => onRemove(entry.key)} className="min-h-11 rounded border border-white/10 px-2 text-base">Remove</button></div></li>; })}</ul>{!entries.length && <p className="rounded border border-dashed border-white/15 p-5 text-sm text-zinc-400">No saved contexts yet.</p>}</div><footer className="border-t border-white/10 p-4">{compareHref ? <a href={compareHref} className="inline-flex min-h-11 w-full items-center justify-center rounded bg-lime-300 px-3 text-base font-black text-black">Compare selected</a> : <span aria-disabled="true" className="inline-flex min-h-11 w-full items-center justify-center rounded bg-white/10 px-3 text-base font-black text-zinc-500">{selectedKeys.length}/2 selected</span>}</footer></aside></div>;
}
