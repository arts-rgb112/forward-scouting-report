...
describe("comparisonReducer", () => {
  it("preserves order, toggles uniquely, and caps at four", () => { let state = { ids: [] as number[], open: false }; for (const id of [3, 1, 4, 2, 5]) state = comparisonReducer(state, { type: "toggle", id }); expect(state.ids).toEqual([3, 1, 4, 2]); });
