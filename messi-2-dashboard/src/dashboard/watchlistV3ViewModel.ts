...
export type WatchlistV3CommonSort = "savedAt" | "name" | "score" | "age";
...
export function watchlistV3Profile(entry: WatchlistV3Entry) {
  return entry.taxonomy === "duel-press-v1"
    ? { name: entry.snapshot.name, clubName: entry.snapshot.club.name, leagueName: entry.snapshot.league.name, position: entry.snapshot.position, archetype: entry.snapshot.archetype, age: entry.snapshot.age, minutes: entry.snapshot.minutes, score: entry.snapshot.score }
    : { name: entry.snapshot.name, clubName: entry.snapshot.clubName, leagueName: entry.snapshot.leagueName, position: entry.snapshot.position, archetype: entry.snapshot.archetype, age: entry.snapshot.age, minutes: entry.snapshot.minutes, score: entry.snapshot.score };
}
...
export function filterAndSortWatchlistV3(entries: readonly WatchlistV3Entry[], filters: WatchlistV3Filters): WatchlistV3Entry[] {
  ...
  return filtered.slice().sort((left, right) => {
    ...
    else comparison = nullableNumber(filters.sort === "score" ? a.score : a.age, filters.sort === "score" ? b.score : b.age, filters.direction);
