...
export type WatchlistV3SortProfile = { name: string; clubName: string; leagueName?: string; position: string; archetype?: "Type A" | "Type B"; age?: number | null; minutes?: number; score?: number; stats?: Partial<Record<WatchlistV3MetricSort, number>> };
export type WatchlistV3SortOverrides = Readonly<Record<string, Partial<WatchlistV3SortProfile>>>;
...
function effectiveProfile(entry: WatchlistV3Entry, overrides?: WatchlistV3SortOverrides): WatchlistV3SortProfile {
  const saved = watchlistV3Profile(entry); const override = overrides?.[entry.key];
  if (!override) return saved;
  return { ...saved, ...override, stats: override.stats ?? saved.stats };
}
...
export function filterAndSortWatchlistV3(entries: readonly WatchlistV3Entry[], filters: WatchlistV3Filters, overrides?: WatchlistV3SortOverrides): WatchlistV3Entry[] {
  const needle = textComparisonKey(filters.query);
  const filtered = entries.filter((entry) => {
    const profile = effectiveProfile(entry, overrides); const searchable = textComparisonKey(`${profile.name} ${profile.clubName} ${profile.leagueName ?? ""} ${profile.position} ${entry.context.season} ${entry.context.mode} ${entry.context.scope ?? ""} ${entry.context.competition}`);
  ...
  return filtered.slice().sort((left, right) => {
    const a = effectiveProfile(left, overrides); const b = effectiveProfile(right, overrides); let comparison = 0;
...
export function watchlistV3Page(entries: readonly WatchlistV3Entry[], filters: WatchlistV3Filters, requestedPage: number, overrides?: WatchlistV3SortOverrides) {
  const filtered = filterAndSortWatchlistV3(entries, filters, overrides); const totalPages = Math.max(1, Math.ceil(filtered.length / WATCHLIST_V3_PAGE_SIZE)); const page = Math.min(Math.max(1, requestedPage), totalPages);
