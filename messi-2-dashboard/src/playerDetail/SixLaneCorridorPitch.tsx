...
import { isPenaltyShot } from "./pitchPenalties";
...
export function SixLaneCorridorPitch({ analysis, layers }: { analysis?: PlayerAnalysis; layers: PitchLayerVisibility }) {
  ...
  const nonPenaltyShots = useMemo(() => validShots ? shots!.filter((shot) => !isPenaltyShot(shot)) : [], [shots, validShots]);
  const penalties = useMemo(() => validShots ? shots!.filter(isPenaltyShot) : [], [shots, validShots]);
  const groups = useMemo(() => groupPitchShots(nonPenaltyShots.map((shot, sourceIndex) => ({ shot, sourceIndex }))), [nonPenaltyShots]);
  const medianXg = useMemo(() => medianObservedXg(nonPenaltyShots), [nonPenaltyShots]);
  ...
  const markerDescription = validShots ? `PK 제외 슛 ${nonPenaltyShots.length}발, 페널티 ${penalties.length}발 별도` : "서버 슈팅 스냅샷 사용 불가";
  ...
  return <section data-layout="six-lane-corridor-pitch" className="rounded-xl border border-white/10 bg-[#0b1011] p-3" aria-label={COPY.fieldLabel}>
    <div className="grid min-w-0 gap-4 xl:grid-cols-[minmax(0,1fr)_18rem] xl:items-start">
      <figure className="min-w-0" aria-describedby="six-lane-corridor-caption">
        <div className="relative overflow-hidden rounded-lg border border-white/10 bg-[#123A20]" style={{ aspectRatio: "105 / 68" }}>
          <HeatmapCanvas points={validHeat ? spatial!.heatmapPoints : []} enabled={layers.heatmap && validHeat} opacity={.62}/>
          <svg viewBox="-2 -2 109 72" role="img" aria-label={`${COPY.fieldLabel}. ${markerDescription}`} className="h-full w-full">
            {LANES.map((lane) => {
              const y = lineY(lane.high);
              const height = lineY(lane.low) - y;
              return <g key={lane.id} data-lane={lane.id}><rect x="0" y={y} width="105" height={height} fill={lane.fill} fillOpacity={lane.opacity}/><text x="4" y={y + height / 2} fill="#FFFFFF" fillOpacity=".72" fontSize="3.2" fontWeight="800" dominantBaseline="middle">{lane.id} · {lane.label}</text></g>;
            })}
            <PitchLines />
            <GuardiolaDepthGrid />
            ...
            {layers.trajectories && <g data-layer="shot-trajectories-2d" fill="none" pointerEvents="none">{nonPenaltyShots.map((shot, index) => shot.trajectory ? <path key={index} d={`M${world(shot).x.toFixed(4)} ${world(shot).y.toFixed(4)}L${(shot.trajectory.endX * 1.05).toFixed(4)} ${((100 - shot.trajectory.endY) * .68).toFixed(4)}`} stroke="#E2E8F0" strokeOpacity=".24" strokeWidth=".28" vectorEffect="non-scaling-stroke"/> : null)}</g>}
            {layers.markers && groups.map((group) => {
              const point = world(group.shot);
              const radius = pitchMarkerRadius(group.shot.xg, medianXg) * .17;
              return <g key={group.key} transform={`translate(${point.x.toFixed(4)} ${point.y.toFixed(4)})`} aria-label={`${group.count}발 슈팅 마커`}><PitchShotMarker outcome={group.outcome} radius={radius} count={group.count} outcomeCounts={group.outcomeCounts} /></g>;
            })}
          </svg>
