...
const panel = "min-w-0 w-full max-w-[560px] overflow-hidden rounded-2xl border border-[#252d2e] bg-[#101516] p-6";
...
function TailTrack({ item }: { item: TacticalSummaryV2Readout }) {
  if (item.cohortState === "unavailable" || item.percentileScore === null) return <p className="text-[11px] text-[#949f9f]">{COPY.noTrack}</p>;
  ...
  return <div className="relative h-[30px] w-full" data-track-direction={below ? "below" : "above"} role="progressbar" aria-label={`${metricLabel(item)} 서버 제공 백분위`} aria-valuemin={0} aria-valuemax={100} aria-valuenow={percentile}>
    ...
    <span className="absolute left-1/2 top-[17px] -translate-x-1/2 text-[9px] leading-none text-[#949f9f]" aria-hidden="true">{COPY.average}</span>
  </div>;
}
function Notices({ item }: { item: TacticalSummaryV2Readout }) {
  ...
  return messages.length ? <div className="space-y-1 text-[11px] text-[#f5b247]" aria-live="polite">{messages.map((message) => <p key={message}>{message}</p>)}</div> : null;
}
function MetricCaption({ item, cohort }: { item: TacticalSummaryV2Readout; cohort: string }) {
  if (item.cohortState === "unavailable") return <p className="text-[11px] text-[#949f9f]">{reasonLabel(item)}</p>;
  return <p className="text-[11px] text-[#949f9f]">{cohort} 중앙값 {baseline(item)} · {signedDelta(item)}</p>;
}
function MainMetric({ sectionLabel, item, cohort, secondary }: { sectionLabel: string; item: TacticalSummaryV2Readout; cohort: string; secondary?: TacticalSummaryV2Readout }) {
  ...
  return <article className="flex min-w-0 flex-col gap-2" aria-label={`${sectionLabel} 전술 지표`}>
    <div className="flex items-center justify-between"><h3 className="text-[11px] font-semibold tracking-[0.6px] text-[#949f9f]">{sectionLabel}</h3><b className={`text-xs ${below ? "text-[#fa6e7a]" : "text-[#b5f052]"}`}>{tailLabel(item)}</b></div>
    <div className="flex items-baseline gap-2.5 text-[#f5f8f7]"><span className="text-[13px]">{metricLabel(item)}</span><strong className="text-[30px] leading-none tabular-nums">{metricValue(item)}</strong></div>
    <TailTrack item={item}/><MetricCaption item={item} cohort={cohort}/>
    {secondary && <p className="text-[11px] text-[#949f9f] opacity-75">다음 레인 · {metricLabel(secondary)} {metricValue(secondary)} · 중앙값 {baseline(secondary)} · {signedDelta(secondary)} · {tailLabel(secondary)}</p>}
    <Notices item={item}/>
...
function ActivityAxis({ item, cohort }: { item: TacticalSummaryV2Readout; cohort: string }) {
  ...
  return <article className="flex min-w-0 flex-col gap-[7px]" aria-label={`${metricLabel(item)} 전술 지표`}>
    <div className="flex items-baseline justify-between gap-3"><div className="flex min-w-0 items-baseline gap-2.5 text-[#f5f8f7]"><span className="text-[13px]">{metricLabel(item)}</span><strong className="text-[26px] leading-none tabular-nums">{metricValue(item)}</strong></div><b className={`shrink-0 text-xs ${below ? "text-[#fa6e7a]" : "text-[#b5f052]"}`}>{tailLabel(item)}</b></div>
    <TailTrack item={item}/><MetricCaption item={item} cohort={cohort}/><Notices item={item}/>
  </article>;
}
function Skeleton() {
  return <section className={panel} aria-labelledby="tactical-summary-v2-heading" aria-busy="true"><h2 id="tactical-summary-v2-heading" className="text-lg font-bold text-[#f5f8f7]">{COPY.title}</h2><div className="mt-4 space-y-4">{Array.from({ length: 3 }, (_, index) => <div key={index} className="h-28 animate-pulse rounded-lg bg-white/10 motion-reduce:animate-none"/>)}</div></section>;
}
export function TacticalSummaryV2Panel({ state, onRetry }: { state: TacticalSummaryV2State; onRetry(): void }) {
  ...
  if (state.kind === "error") return <section className={panel} aria-labelledby="tactical-summary-v2-heading"><h2 id="tactical-summary-v2-heading" className="text-lg font-bold text-[#f5f8f7]">{COPY.title}</h2><div role="alert" className="mt-4 flex flex-wrap items-center justify-between gap-3 rounded border border-[#f5b247]/30 bg-[#f5b247]/10 p-3 text-sm text-amber-100"><span>{COPY.loadError}</span><button type="button" onClick={onRetry} className="min-h-11 rounded border border-[#f5b247]/50 px-3 text-sm font-bold focus-visible:ring-2 focus-visible:ring-[#f5b247]">{COPY.retry}</button></div></section>;
  const { data } = state;
  ...
  return <section className={`${panel} flex flex-col gap-4`} aria-labelledby="tactical-summary-v2-heading" data-layout="approved-tactical-summary">
    <header className="flex flex-col gap-1.5"><h2 id="tactical-summary-v2-heading" className="text-lg font-bold text-[#f5f8f7]">{COPY.title}</h2><p className="text-xs text-[#949f9f]">{COPY.comparison} · {scopeLabel(data.cohortKey.scope, data.cohortKey.mode)} · {data.cohortKey.rawPosition} · N={data.cohortPopulation} · {COPY.server}</p></header>
    <div className="flex flex-col gap-4"><MainMetric sectionLabel={COPY.positioning} item={data.positioning} cohort={cohort}/>{movement && <MainMetric sectionLabel={COPY.movement} item={movement} cohort={cohort} secondary={nextMovement}/>}<section className="flex min-w-0 flex-col gap-3.5" aria-labelledby="activity-range-heading"><div className="flex items-center justify-between"><h3 id="activity-range-heading" className="text-[11px] font-semibold tracking-[0.6px] text-[#949f9f]">{COPY.activity}</h3><b className="text-xs text-[#45d6ed]">{range.roleLabel === "unavailable" ? COPY.unavailableRole : range.roleLabel}</b></div><ActivityAxis item={range.frontBackActivityRange} cohort={cohort}/><ActivityAxis item={range.leftRightActivityRange} cohort={cohort}/><p className="text-[10px] text-[#949f9f] opacity-70">{data.disclosure}</p></section></div>
  </section>;
}
