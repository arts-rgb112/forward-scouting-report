...
export function SpatialPitch({ analysis, contextIdentity = "", forcedMode, embedded = false, layers = DEFAULT_PITCH_LAYERS, fullActivityHeatmap, boxSubregion, nativePitchEvents }: {
   ...
   * relabelled" — never threaded into the 2D `LegacySpatialPitchFigure` below. */
  ...
  const [shotSource, setShotSource] = useState<"sportsapi" | "fotmob">(() => nativePitchEvents ? "sportsapi" : "fotmob");
  ...
  // The 3D route receives an explicit native bundle.  Its renderer owns the
  // source switch, so legacy FotMob totals/30-zone summaries must not leak
  // into the default SportsAPI view while the native request loads or fails.
  const nativeThree = mode === "perspective" && nativePitchEvents !== undefined;
  const nativeSourceSelected = nativeThree && shotSource === "sportsapi";
  useEffect(() => { setShotSource(nativePitchEvents ? "sportsapi" : "fotmob"); }, [contextIdentity]);
  ...
  const nativeShotSentence = nativePitchEvents?.kind === "ready" && nativePitchEvents.data.bodyParts.completeness === "unavailable" ? "SportsAPI 슛 원천이 관측되지 않았습니다. 관측된 0이나 FotMob으로 대체하지 않습니다." : nativePitchEvents?.kind === "ready" ? `SportsAPI 동일 응답 슛 ${nativePitchEvents.data.events.length}개 · 데이터 없음과 관측된 0은 구분합니다.` : nativePitchEvents?.kind === "loading" ? "SportsAPI 슛 원천을 불러오는 중입니다." : "SportsAPI 슛 원천을 사용할 수 없습니다. FotMob으로 자동 대체하지 않습니다.";
  const shotSentence = nativeSourceSelected ? nativeShotSentence : `${shotState} · 데이터 없음과 관측된 0은 구분합니다.`;
