...
function IndicatorCard({ indicator }: { indicator: DuelPressV2DetailMetrics["contextIndicators"][number] }) {
  ...
  return <article className="min-w-0 rounded-xl border border-white/10 bg-[#101415] p-3"><div className="flex items-center justify-between gap-3"><h3 className="text-sm font-black text-white">{label}</h3><Tooltip label={label} content={value ? <MetricTooltip metric={indicator.metric} item={value} slot="value"/> : "데이터 없음"}><b className={`inline-flex min-w-12 items-center justify-center rounded border px-2 py-1 font-mono text-sm ${tone.badge}`}>{score ?? "—"}</b></Tooltip></div><div role="progressbar" aria-label={`${label} 백분위`} aria-valuemin={0} aria-valuemax={99} {...(score === null ? {} : { "aria-valuenow": score })} className="mt-3 h-1.5 overflow-hidden rounded bg-white/10"><span className={`block h-full rounded ${tone.fill}`} style={{ width: `${score ?? 0}%` }} /></div></article>;
}
...
export function DuelPressV2DetailReadoutBoard({ data, layout = "page" }: { data: DuelPressV2DetailMetrics; layout?: "page" | "rail" }) {
  ...
  return <section aria-label="Duel press v2 detailed stats board" data-taxonomy="duel-press-v2" className="min-w-0"><div className="flex flex-wrap items-baseline justify-between gap-2"><h2 className="text-lg font-black">상세 스탯 보드</h2><span className="text-[10px] tracking-[0.16em] text-zinc-500">원시값 · 서버 백분위</span></div><p className="mt-1 text-[11px] text-zinc-500">오른쪽 색상 숫자는 동일 시즌·모드·스코프 코호트의 백분위입니다.</p><div className={grid}>{data.categories.map((category) => <CategoryCard key={category.id} category={category}/>)}</div><div className="mt-3 grid min-w-0 grid-cols-1 gap-3 sm:grid-cols-2">{data.contextIndicators.map((indicator) => <IndicatorCard key={indicator.id} indicator={indicator}/>)}</div></section>;
}
