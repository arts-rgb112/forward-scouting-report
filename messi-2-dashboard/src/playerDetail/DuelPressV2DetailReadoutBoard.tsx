...
export function DuelPressV2CategorySummary({ data }: { data: DuelPressV2DetailMetrics }) {
  return <section aria-label="Duel press v2 category summary" data-taxonomy="duel-press-v2" className="min-w-0"><div className="flex flex-wrap items-baseline justify-between gap-2"><h2 className="type-title font-black">상세 스탯 보드</h2><span className="type-caption tracking-[0.12em] text-zinc-400">서버 백분위</span></div><p className="mt-1 text-base text-zinc-400">기본 화면은 카테고리 점수만 보여주며, 전체 지표는 아래에서 펼칠 수 있습니다.</p><div data-detail-level="summary" className="mt-4 grid min-w-0 grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-3">{data.categories.map((category) => <CategoryCard key={category.id} category={category} summary/>)}</div></section>;
}

export function DuelPressV2CategoryDetails({ data, layout = "page" }: { data: DuelPressV2DetailMetrics; layout?: "page" | "rail" }) {
  ...
  return <section aria-label="Duel press v2 detailed stats board"><details data-detail-level="expanded" className="rounded-xl border border-white/10 bg-black/20"><summary className="min-h-12 cursor-pointer px-4 py-3 text-base font-black focus-visible:ring-2 focus-visible:ring-lime-300">총량·/90·비율 전체 지표 펼치기</summary><div className={`border-t border-white/10 p-3 ${fullGrid}`}>{data.categories.map((category) => <CategoryCard key={category.id} category={category}/>)}</div></details></section>;
}

export function DuelPressV2DataAudit({ data }: { data: DuelPressV2DetailMetrics }) {
  return <section aria-label="원본 감사 정보"><h3 className="type-label font-black">데이터 품질 · 원본 감사</h3><div className="mt-2 grid min-w-0 grid-cols-1 gap-3 sm:grid-cols-2">{data.contextIndicators.map((indicator) => <IndicatorCard key={indicator.id} indicator={indicator}/>)}</div></section>;
}

export function DuelPressV2DetailReadoutBoard({ data, layout = "page" }: { data: DuelPressV2DetailMetrics; layout?: "page" | "rail" }) {
  return <section aria-label="Duel press v2 detailed stats board" data-taxonomy="duel-press-v2" className="min-w-0"><DuelPressV2CategorySummary data={data}/><div className="mt-4"><DuelPressV2CategoryDetails data={data} layout={layout}/></div><div className="mt-4"><DuelPressV2DataAudit data={data}/></div></section>;
}
