...
function Tooltip({ label, children, content }: { label: string; children: ReactNode; content: ReactNode }) {
  ...
  return <span className="relative inline-flex min-w-0" onPointerEnter={() => setOpen(true)} onPointerLeave={() => setOpen(false)}>
    ...
    {open && <span id={id} role="tooltip" className="absolute right-0 top-full z-30 mt-2 w-72 max-w-[calc(100vw-2rem)] rounded border border-white/20 bg-[#101415] p-3 text-[11px] leading-4 text-zinc-200 shadow-xl">{content}</span>}
  </span>;
}
...
function MetricRow({ row }: { row: ReadoutRow }) {
  ...
  return <div data-metric-id={row.metric.id} data-metric-slot={row.slot} className="grid min-w-0 grid-cols-[minmax(0,1fr)_auto_auto] items-center gap-x-2 border-t border-white/10 py-1.5">
    <span className="min-w-0 break-words text-[11px] font-normal text-zinc-400">{row.label}</span>
    <span className="min-w-11 text-right font-mono text-[11px] tabular-nums text-zinc-100">{formatRawValue(row.value)}</span>
    <Tooltip label={row.label} content={<MetricTooltip metric={row.metric} item={row.value} slot={row.slot}/>}><b className={`inline-flex min-w-8 items-center justify-center rounded px-1.5 py-0.5 font-mono text-[10px] font-semibold tabular-nums ${tone.badge}`}>{displayScore ?? "—"}</b></Tooltip>
  </div>;
}
...
function CategoryCard({ category, summary = false }: { category: DuelPressV2Category; summary?: boolean }) {
  ...
  return <article data-taxonomy="duel-press-v2" className="min-w-0 rounded-xl border border-white/10 bg-[#101415] p-3 shadow-sm">
    <div className="flex min-w-0 items-start justify-between gap-3"><div className="min-w-0"><h3 className="truncate text-sm font-black text-white">{title}</h3><p className="mt-1 text-[10px] text-zinc-500">원시값 · 동일 코호트 백분위</p></div><Tooltip label={title} content={<CategoryTooltip category={category}/>}><b className={`inline-flex min-w-11 items-center justify-center rounded border px-2 py-1 font-mono text-lg font-black ${tone.badge}`}>{score ?? "—"}</b></Tooltip></div>
    ...
    {summary ? <div data-detail-level="core" className="mt-3">{category.groups.flatMap(rowsForGroup).slice(0, 4).map((row) => <MetricRow key={`${row.metric.id}-${row.slot}`} row={row}/>)}</div> : category.groups.map((group) => <section key={group.id} className="mt-3 min-w-0" aria-label={duelPressV2GroupLabel(group.id, group.label)}>{hasMultipleGroups && <h4 className="border-b border-white/10 pb-1 text-xs font-bold text-zinc-200">{duelPressV2GroupLabel(group.id, group.label)}</h4>}{rowsForGroup(group).map((row) => <MetricRow key={`${row.metric.id}-${row.slot}`} row={row}/>)}</section>)}
  </article>;
}
...
function IndicatorCard({ indicator }: { indicator: DuelPressV2DetailMetrics["contextIndicators"][number] }) {
  ...
  return <article className="min-w-0 rounded-xl border border-white/10 bg-[#101415] p-3"><div className="flex items-center justify-between gap-3"><div><h3 className="text-sm font-black text-white">{label}</h3><p className="mt-1 text-xs text-zinc-400">{value ? formatRawValue(value) : "—"} · 백분위 {score ?? "—"}</p></div><Tooltip label={label} content={value ? <MetricTooltip metric={indicator.metric} item={value} slot="value"/> : "데이터 없음"}><b className={`inline-flex min-w-12 items-center justify-center rounded border px-2 py-1 font-mono text-sm ${tone.badge}`}>{score ?? "—"}</b></Tooltip></div><div role="progressbar" aria-label={`${label} 백분위`} aria-valuemin={0} aria-valuemax={99} {...(score === null ? {} : { "aria-valuenow": score })} className="mt-3 h-1.5 overflow-hidden rounded bg-white/10"><span className={`block h-full rounded ${tone.fill}`} style={{ width: `${score ?? 0}%` }} /></div></article>;
}
...
export function DuelPressV2DetailReadoutBoard({ data, layout = "page" }: { data: DuelPressV2DetailMetrics; layout?: "page" | "rail" }) {
  ...
  return <section aria-label="Duel press v2 detailed stats board" data-taxonomy="duel-press-v2" className="min-w-0"><div className="flex flex-wrap items-baseline justify-between gap-2"><h2 className="text-lg font-black">상세 스탯 보드</h2><span className="text-xs tracking-[0.12em] text-zinc-400">원시값 · 서버 백분위</span></div><p className="mt-1 text-sm text-zinc-400">기본 화면은 카테고리별 핵심 지표를 보여주며, 전체 지표는 아래에서 펼칠 수 있습니다.</p><div data-detail-level="summary" className="mt-4 grid min-w-0 grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-3">{data.categories.map((category) => <CategoryCard key={category.id} category={category} summary/>)}</div><details data-detail-level="expanded" className="mt-4 rounded-xl border border-white/10 bg-black/20"><summary className="min-h-12 cursor-pointer px-4 py-3 text-sm font-black focus-visible:ring-2 focus-visible:ring-lime-300">총량·/90·비율 전체 지표 펼치기</summary><div className={`border-t border-white/10 p-3 ${fullGrid}`}>{data.categories.map((category) => <CategoryCard key={category.id} category={category}/>)}</div></details><section aria-label="원본 감사 정보" className="mt-4"><h3 className="text-sm font-black">데이터 품질 · 원본 감사</h3><div className="mt-2 grid min-w-0 grid-cols-1 gap-3 sm:grid-cols-2">{data.contextIndicators.map((indicator) => <IndicatorCard key={indicator.id} indicator={indicator}/>)}</div></section></section>;
}
