...
function OverviewSeasonRow({ row, player, analysis }: { row: SeasonOverviewRow; player: Player; analysis?: PlayerAnalysis }) {
  ...
  const content = <><div className="min-w-0"><div className="flex min-w-0 items-center gap-1.5"><b className="truncate type-label text-zinc-100">{row.context.season}</b>{row.selected && <span className="rounded border border-white/20 px-1.5 py-0.5 type-caption text-zinc-300">현재</span>}</div><p className="mt-0.5 truncate type-caption text-zinc-500">{contextText(row.context)}</p></div><div className="flex shrink-0 items-center gap-2"><span className="hidden h-1 w-9 overflow-hidden rounded bg-white/10 xl:block" aria-hidden="true"><span className="block h-full rounded" style={{ backgroundColor: color, width: `${Math.min(100, Math.max(0, row.score / 99 * 100))}%` }} /></span><b className="type-label tabular-nums text-zinc-100">{score}</b></div></>;
  const rowClass = `grid min-h-12 min-w-40 grid-cols-[minmax(0,1fr)_auto] items-center gap-x-3 rounded-lg border px-3 py-1.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-lime-300 xl:min-w-0 xl:rounded-none xl:border-y-0 xl:border-r-0 xl:border-l-2 xl:px-3 ${row.selected ? "border-l-white/70 bg-white/[.07]" : "border-l-white/10 bg-transparent hover:border-l-white/40 hover:bg-white/[.035]"}`;
...
function PlayerIdentity({ player, analysis, selected }: { player: Player; analysis?: PlayerAnalysis; selected: DatasetRouteState }) {
  ...
  return <section aria-labelledby="overview-profile-heading" className="relative isolate min-w-0 overflow-hidden px-4 py-5 sm:px-6 xl:col-start-2 xl:row-start-1 xl:py-7">
    ...
    <div className="relative mt-5 grid min-w-0 grid-cols-[minmax(0,1fr)_auto] gap-4 xl:grid-cols-[10.5rem_minmax(0,1fr)_auto] xl:items-center xl:gap-6">
      <div className="row-span-2 hidden h-48 w-[10.5rem] overflow-hidden rounded-[1.3rem] bg-[#0a1019] ring-1 ring-white/10 xl:block">{player.face ? <img src={player.face} alt={`${player.name} 선수 사진`} className="h-full w-full object-cover object-top" /> : <span className="grid h-full place-items-center text-5xl font-black text-zinc-600" aria-hidden="true">{player.name[0]}</span>}</div>
      ...
      <div className="flex min-w-0 items-center gap-3 xl:col-start-2"><div className="grid size-[4.75rem] shrink-0 overflow-hidden rounded-2xl bg-[#0a1019] ring-1 ring-white/10 xl:hidden">{player.face ? <img src={player.face} alt={`${player.name} 선수 사진`} className="h-full w-full object-cover object-top" /> : <span className="grid h-full place-items-center text-2xl font-black text-zinc-600" aria-hidden="true">{player.name[0]}</span>}</div><div className="min-w-0 type-caption text-zinc-400"><p>대회 전체 <b className="text-zinc-100">{player.rank}위</b></p><p className="mt-1">동포지션 <b className="text-zinc-100">{positionRank === null ? "—" : `${positionRank}위`}</b> / {positionPopulation === null ? "—" : `${positionPopulation}명`}</p></div></div>
    </div>
...
export function PlayerOverview({ player, analysis, selected, history, data, categoryState }: { player: Player; analysis?: PlayerAnalysis; selected: DatasetRouteState; history: PlayerOverviewHistory; data?: DuelPressV2DetailMetrics; categoryState?: OverviewReadoutState }) {
  ...
  return <section data-layout="player-overview" aria-label="선수 첫 화면 요약" className="grid min-w-0 overflow-hidden rounded-[1.75rem] border border-white/10 bg-[#0b1220] shadow-[0_24px_80px_rgba(0,0,0,.26)] xl:grid-cols-[12rem_minmax(0,1fr)_22rem] xl:items-stretch">
