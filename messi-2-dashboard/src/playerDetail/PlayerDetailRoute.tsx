...
export const v2ContextMatches = (value: DuelPressV2DetailMetrics, id: number, dataset: DatasetRouteState) => value.context.playerId === id
  && value.context.season === dataset.season
  && value.context.mode === dataset.mode
  && (dataset.mode === "league"
...
export function PlayerTierCard({ player, analysis, quality, detailReadouts, renewedDetailRequested = false }: { player: Player; analysis?: PlayerAnalysis; quality: QualityDisplay; detailReadouts?: DuelPressDetailReadoutEnvelope; renewedDetailRequested?: boolean }) {
  ...
  return <section className={`${panel} dossier-card relative isolate overflow-hidden bg-gradient-to-br ${dossierGradient(player.tier.code)} ${tier.className}`} aria-labelledby="player-tier-heading">
    ...
    <h2 id="player-tier-heading" className="sr-only">Player dossier</h2><div className="relative flex items-start justify-between gap-3"><div><p className="type-caption font-black tracking-[0.24em] text-zinc-300">M.E.S.S.I. DOSSIER</p><p className="mt-2 text-6xl font-black leading-none tabular-nums">{score}</p><p className="mt-1 type-caption font-bold uppercase tracking-[0.18em] text-zinc-300">Selected score · floored</p></div><TierBadge tier={player.tier} /></div>
    <div className="relative mt-5 flex items-center gap-3"><div className="h-20 w-16 shrink-0 overflow-hidden rounded border border-white/25 bg-black/30 shadow-inner">{player.face ? <img src={player.face} alt={`${player.name} portrait`} className="h-full w-full object-cover" /> : <span className="grid h-full place-items-center text-2xl" aria-hidden="true">{player.name[0]}</span>}</div><div className="min-w-0"><p className="truncate text-xl font-black text-white">{player.name}</p><p className="truncate text-base text-zinc-200">{player.club.name} · {player.position}</p><p className="mt-1 text-base text-zinc-300">{player.age === null ? "Age unavailable" : `Age ${player.age}`} · {player.minutes} min</p>{player.nation?.icon && <p className="mt-2 flex items-center gap-1.5 type-caption text-zinc-200"><img src={player.nation.icon} alt="" className="h-3.5 w-5 object-contain"/>{player.nation.name}</p>}</div></div>
    {!renewedDetailRequested && <dl className="relative mt-5 grid grid-cols-6 gap-1 border-t border-white/15 pt-3">{detailReadouts ? detailReadouts.categories.map((category, index) => <div key={category.id} className="min-w-0 rounded border border-white/10 bg-black/25 p-1.5 text-center"><dt className="type-caption font-black tracking-wide" aria-label={category.label}>{String(index + 1).padStart(2, "0")}</dt><dd className={`mt-0.5 text-base font-black ${category.score === null ? "text-zinc-500" : getScoreBand(category.score).dotClassName}`}>{category.score ?? "—"}</dd></div>) : detailMetrics.map(([id, short, label]) => <div key={id} className="min-w-0 rounded border border-white/10 bg-black/25 p-1.5 text-center"><dt className="type-caption font-black tracking-wide" aria-label={label}>{short}</dt><dd className={`mt-0.5 text-base font-black ${getScoreBand(player.stats[id]).dotClassName}`}>{player.stats[id]}{metricIsImputed(quality, id) && <span title="Imputed server value" aria-label="imputed" className="ml-0.5 text-amber-200">*</span>}</dd></div>)}</dl>}
  </section>;
}
...
export function SeasonScorePanel({ player, analysis, selected, history }: { player: Player; analysis?: PlayerAnalysis; selected: DatasetRouteState; history: PlayerHistoryState }) {
  ...
  return <section className={`${panel} season-rail border-l-2 border-l-white/30`} aria-labelledby="season-score-heading"><h2 id="season-score-heading" className="text-sm font-black">Season score rail</h2><div className="mt-2 flex items-center gap-2"><span className="text-4xl font-black tabular-nums">{wholeScore(player, analysis)}</span><TierBadge tier={player.tier} /></div><p className="text-base text-zinc-400">{contextLabel(selected)} · {selected.season}</p><p className="mt-4 border-y border-white/10 py-2 type-caption text-zinc-300">Retrieved-score range: {Math.min(...range).toFixed(1)}–{Math.max(...range).toFixed(1)}{history.entries.length ? "" : " (selected context only)"}</p><ol className="mt-3 space-y-1.5 text-base">{history.loading ? Array.from({ length: 4 }, (_, index) => <li key={index} className="h-8 animate-pulse rounded bg-white/10 motion-reduce:animate-none" />) : rows.map((row, index) => <li key={`${row.context.season}-${row.context.mode}`} className="flex items-center justify-between gap-2 border-l-2 border-white/15 bg-black/20 px-2 py-1.5"><span className="min-w-0"><b className="block truncate">{index === 0 ? "Selected · " : ""}{row.context.season}</b><span className="type-caption text-zinc-400">{row.context.mode === "league" ? `League · ${row.context.scope}` : `Europe · ${row.context.competition.toUpperCase()}`}</span></span><b className="shrink-0 tabular-nums">{index === 0 ? wholeScore(player, analysis) : row.player.score.toFixed(1)}</b></li>)}</ol>{!history.loading && history.requestedSeasons > 0 && <p className="mt-3 type-caption text-zinc-400">One best server context per season; top {Math.min(4, Math.max(0, rows.length - 1))} of {history.requestedSeasons} historical seasons.</p>}{history.failed > 0 && <p aria-live="polite" className="mt-2 text-base text-amber-200">Partial history: {history.failed} context{history.failed === 1 ? "" : "s"} unavailable; range reflects retrieved scores only.</p>}</section>;
}
...
export function PlayerDetailRoute({ id, dataset, config: providedConfig, afterPanels, duelPressDetailRequested = false, duelPressV2Requested = false }: { id: number; dataset: DatasetRouteState; config?: MessiApiConfig; afterPanels?: ReactNode; duelPressDetailRequested?: boolean; duelPressV2Requested?: boolean }) {
  ...
  const currentV2Readouts = v2Readouts && v2ContextMatches(v2Readouts, id, dataset) ? v2Readouts : undefined;
  const v2ReadoutErrorForCurrentContext = v2Readouts && !currentV2Readouts ? undefined : v2ReadoutError;
  const detailBoard = duelPressV2Requested ? currentV2Readouts ? <DuelPressV2CategoryDetails data={currentV2Readouts} layout="page"/> : <DuelPressV2DetailReadoutUnavailable loading={!v2ReadoutErrorForCurrentContext} message={v2ReadoutErrorForCurrentContext} onRetry={() => setReadoutRetry((value) => value + 1)}/> : duelPressDetailRequested ? readouts ? <details className="rounded-xl border border-white/10 bg-black/20"><summary className="min-h-12 cursor-pointer px-4 py-3 text-base font-black focus-visible:ring-2 focus-visible:ring-lime-300">상세 스탯 보드</summary><div className="border-t border-white/10 p-3"><DuelPressDetailReadoutBoard data={readouts} layout="page"/></div></details> : <DuelPressDetailReadoutUnavailable loading={!readoutError} message={readoutError} onRetry={() => setReadoutRetry((value) => value + 1)}/> : undefined;
  const dataAuditBoard = duelPressV2Requested && currentV2Readouts ? <DuelPressV2DataAudit data={currentV2Readouts}/> : undefined;
  ...
  const overviewCategoryState = currentV2Readouts ? "ready" : duelPressV2Requested ? v2Readouts ? "loading" : v2ReadoutErrorForCurrentContext ? "error" : "loading" : "unavailable";
  return <main id="main-content" className="mx-auto max-w-[1580px] overflow-x-hidden p-3 text-zinc-100 sm:p-6"><a href={back} className="inline-flex min-h-11 items-center text-lime-300 focus-visible:ring-2">← {ROUTE_COPY.back}</a><h1 ref={titleRef} tabIndex={-1} className="sr-only outline-none">{detail.player.name} 선수 상세</h1><PlayerDetailDossierLayout player={detail.player} analysis={detail.analysis} quadrant={quadrant} quality={quality} history={history} config={config} dataset={dataset} afterPanels={afterPanels} detailReadoutBoard={detailBoard} dataAuditBoard={dataAuditBoard} detailReadouts={readouts} v2Readouts={currentV2Readouts} overviewCategoryState={overviewCategoryState} renewedDetailRequested={duelPressDetailRequested || duelPressV2Requested}/></main>;
}
