...
import { axisDetail, detailMetrics, metricProfile, radarAxisLabels, radarValues, seasonScoreRows, selectedScore, tacticalCopy, wholeScore } from "./playerDetailViewModel";
...
export function VolumeBenchmarkRadar({ player, analysis }: { player: Player; analysis?: PlayerAnalysis }) { const values = radarValues(analysis); const all = values.every((value): value is number => typeof value === "number"); const n = 6; const center = 70; const point = (value: number, index: number) => { const angle = -Math.PI / 2 + index * (Math.PI * 2 / n); const radius = value * .55; return `${center + Math.cos(angle) * radius},${center + Math.sin(angle) * radius}`; }; return <section className={panel} aria-labelledby="volume-radar-heading"><h2 id="volume-radar-heading" className="text-sm font-black">Volume benchmark radar</h2><svg viewBox="0 0 140 140" className="mx-auto mt-2 w-full max-w-sm" role="img" aria-label="Six-axis player volume radar; 8-league average benchmark unavailable"><g className="fill-none stroke-zinc-600" strokeWidth=".5">{[25, 50, 75, 100].map((value) => <polygon key={value} points={Array.from({ length: n }, (_, index) => point(value, index)).join(" ")} />)}{Array.from({ length: n }, (_, index) => <line key={index} x1="70" y1="70" x2={point(100, index).split(",")[0]} y2={point(100, index).split(",")[1]} />)}</g>{all && <polygon points={values.map(point).join(" ")} className="fill-lime-300/25 stroke-lime-300" strokeWidth="1.5" />}{radarAxisLabels.map((label, index) => { const [x, y] = point(116, index).split(","); return <text key={label} x={x} y={y} textAnchor="middle" className="fill-zinc-300 text-[6px]">{label}</text>; })}</svg><p className="text-xs text-lime-200">Player: {player.name}</p><p className="text-xs text-violet-200">8-league avg · 8-league average benchmark unavailable; server endpoint not yet shipped.</p><ul className="mt-2 grid grid-cols-2 gap-1 text-xs text-zinc-400">{radarAxisLabels.map((label, index) => <li key={label}>{label}: {values[index] ?? "unavailable"}</li>)}</ul></section>; }

export function PlayerDetailRoute({ id, dataset, config: providedConfig }: { id: number; dataset: DatasetRouteState; config?: MessiApiConfig }) {
