...
export function PlayerTierCard({ player, analysis, quality, detailReadouts, renewedDetailRequested = false }: { player: Player; analysis?: PlayerAnalysis; quality: QualityDisplay; detailReadouts?: DuelPressDetailReadoutEnvelope; renewedDetailRequested?: boolean }) {
  ...
  return <section className={`${panel} dossier-card relative isolate overflow-hidden bg-gradient-to-br ${dossierGradient(player.tier.code)} ${tier.className}`} aria-labelledby="player-tier-heading">
    ...
    <dl className="relative mt-5 grid grid-cols-6 gap-1 border-t border-white/15 pt-3">{detailReadouts ? detailReadouts.categories.map((category, index) => <div key={category.id} className="min-w-0 rounded border border-white/10 bg-black/25 p-1.5 text-center"><dt className="text-[9px] font-black tracking-wide" aria-label={category.id}>{String(index + 1).padStart(2, "0")}</dt><dd className={`mt-0.5 text-xs font-black ${category.score === null ? "text-zinc-500" : getScoreBand(category.score).dotClassName}`}>{category.score ?? "—"}</dd></div>) : renewedDetailRequested ? ["박스 밖 슈팅", "박스 안 슈팅", "온볼 전개 영향력", "통합 경합", "오프 더 볼", "전방 압박 효율"].map((label, index) => <div key={label} className="min-w-0 rounded border border-white/10 bg-black/25 p-1.5 text-center"><dt className="text-[9px] font-black tracking-wide" aria-label={label}>{String(index + 1).padStart(2, "0")}</dt><dd className="mt-0.5 text-xs font-black text-zinc-500">—</dd></div>) : detailMetrics.map(([id, short, label]) => <div key={id} className="min-w-0 rounded border border-white/10 bg-black/25 p-1.5 text-center"><dt className="text-[9px] font-black tracking-wide" aria-label={label}>{short}</dt><dd className={`mt-0.5 text-xs font-black ${getScoreBand(player.stats[id]).dotClassName}`}>{player.stats[id]}{metricIsImputed(quality, id) && <span title="Imputed server value" aria-label="imputed" className="ml-0.5 text-amber-200">*</span>}</dd></div>)}</dl>
  </section>;
}
...
export function PlayerDetailDossierLayout({ player, analysis, quadrant, quality, history, config, dataset, afterPanels, detailReadoutBoard, detailReadouts, renewedDetailRequested = false }: { player: Player; analysis?: PlayerAnalysis; quadrant?: TacticalQuadrant; quality: QualityDisplay; history: PlayerHistoryState; config?: MessiApiConfig; dataset: DatasetRouteState; afterPanels?: ReactNode; detailReadoutBoard?: ReactNode; detailReadouts?: DuelPressDetailReadoutEnvelope; renewedDetailRequested?: boolean }) {
  ...
  return <><div data-layout="dossier-season-analysis" className="mt-4 grid min-w-0 gap-4 md:grid-cols-2 lg:grid-cols-[minmax(272px,300px)_minmax(240px,280px)_minmax(0,1fr)]"><PlayerTierCard player={player} analysis={analysis} quality={quality} detailReadouts={detailReadouts} renewedDetailRequested={renewedDetailRequested}/><SeasonScorePanel player={player} analysis={analysis} selected={dataset} history={history}/><section className="min-w-0 md:col-span-2 lg:col-span-1 rounded-xl border border-white/10 bg-[#0d1112] p-2 shadow-sm" aria-label="Tactical and spatial analysis"><TacticalSummary player={player} analysis={analysis} quadrant={quadrant} quality={quality} config={config} dataset={dataset}/><div className="mt-2"><SpatialPitch analysis={analysis} contextIdentity={spatialContextIdentity}/></div></section></div>{!analysis && <p className="mt-4 rounded border border-amber-300/30 bg-amber-300/10 p-3 text-sm text-amber-100">Server analysis is unavailable; no client-side analysis has been invented.</p>}<div data-layout="sectors-radar" className="mt-4 grid min-w-0 gap-4 lg:grid-cols-[5fr_7fr]">{detailReadoutBoard ?? <PercentileProfile player={player} analysis={analysis} quality={quality}/>}<Benchmark player={player} config={config} dataset={dataset}/></div>{afterPanels}</>;
}
...
export function PlayerDetailRoute({ id, dataset, config: providedConfig, afterPanels, duelPressDetailRequested = false }: { id: number; dataset: DatasetRouteState; config?: MessiApiConfig; afterPanels?: ReactNode; duelPressDetailRequested?: boolean }) {
  ...
  const detailBoard = duelPressDetailRequested ? readouts ? <DuelPressDetailReadoutBoard data={readouts}/> : <DuelPressDetailReadoutUnavailable loading={!readoutError} message={readoutError} onRetry={() => setReadoutRetry((value) => value + 1)}/> : undefined;
