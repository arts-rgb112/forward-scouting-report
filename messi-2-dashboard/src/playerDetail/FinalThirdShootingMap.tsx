...
import type { FinalThirdShotMapData } from "../api/finalThirdShotMapContracts";
...
function ChartViews({ data, resourceKey, id }: { data: FinalThirdShotMapData; resourceKey: string; id: string }) {
...
export function FinalThirdShootingMap({ config, playerId, dataset }: { config?: MessiApiConfig; playerId: number; dataset: DatasetRouteState }) {
  ...
  return <section data-layout="final-third-shot-chart" className={`${panel} mt-4 w-full`} aria-labelledby={`final-third-${id}`}><div className="flex flex-wrap items-start justify-between gap-3"><div><h2 id={`final-third-${id}`} className="text-sm font-black">Final Third Shot Chart</h2><p className="mt-1 text-xs text-zinc-400">Authoritative front-two-depth positional zones and goal-mouth endpoints only.</p></div>{data && <p role="status" aria-live="polite" className="text-xs text-zinc-300">{state.kind === "partial" ? "Partial coverage" : state.kind === "observed-zero" ? "Observed zero attempts" : state.kind === "unavailable" ? "Unavailable" : "Ready"}</p>}</div>
    ...
    {data && state.kind !== "unavailable" && <><ChartViews key={resourceKey} data={data} resourceKey={resourceKey} id={id}/>{state.kind === "partial" && <p className="mt-3 text-xs text-amber-100">Partial coverage: {data.partialCoverage.map((issue) => `${issue.zoneId ?? issue.shotId}: ${issue.reason}`).join("; ")}</p>}<p className="mt-3 text-xs text-zinc-400">Context: {data.gridVersion}; {data.attackDirection.replaceAll("_", " ")}; depths {data.includedDepths.join(", ")}. No browser-derived shot, endpoint, quality, or fallback data is used.</p></>}
  </section>;
}
