...
describe("use volume benchmark", () => {
  ...
  it("makes no request while disabled", () => { vi.stubEnv("VITE_VOLUME_BENCHMARK_ENABLED", "false"); const hook=renderHook(()=>useVolumeBenchmark(config,1,league)); expect(hook.result.current.state.kind).toBe("disabled"); expect(transport.fetch).not.toHaveBeenCalled(); });
