...
describe("TacticalSummaryV2Panel",()=>{
  it("renders observed server values, two activity axes, and the exact disclosure",()=>{render(panel(tacticalSummaryV2Fixture()));expect(screen.getAllByText("종적 왕복형").length).toBe(2);expect(screen.getByText("전후 활동폭")).toBeInTheDocument();expect(screen.getByText("좌우 활동폭")).toBeInTheDocument();expect(screen.getByText("활동 폭은 위치 분포의 범위이며 이동 거리가 아닙니다.")).toBeInTheDocument();expect(screen.getAllByText(/기준선 기여 N=284/).length).toBeGreaterThan(0);});
  it("keeps low-sample values and reports both cohort and coordinate causes without recalculation",()=>{render(panel(tacticalSummaryV2Fixture("low_sample")));expect(screen.getAllByText(/비교 대상 N=7명/).length).toBeGreaterThan(0);cleanup();render(panel(tacticalSummaryV2SubjectCoordinateLowFixture()));expect(screen.getByText(/측정 표본 부족 \(좌표 N=42개\)/)).toBeInTheDocument();});
  it("uses the rose below-median label and left-sided bar",()=>{const {container}=render(panel(tacticalSummaryV2Fixture()));expect(screen.getByText("하위 35%")).toHaveClass("text-rose-200");expect(container.querySelector('[style*="right: 50%"]')).not.toBeNull();});
