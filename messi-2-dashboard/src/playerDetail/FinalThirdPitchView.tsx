import type { FinalThirdShotMapData, FinalThirdZone } from "../api/finalThirdShotMapContracts";
import { finalThirdPlanCrop, PlanPitchGeometry, PLAN_VERTICAL_TRANSFORM_Y, projectPlan } from "./SpatialPitch";

const OUTER_RADIUS = 52;
const zoneColor = (zone: FinalThirdZone, data: FinalThirdShotMapData) => zone.qualityScore === null ? "#3f3f46" : `hsl(${Math.round(Math.max(0, Math.min(1, (zone.qualityScore - data.qualityScale.min) / (data.qualityScale.max - data.qualityScale.min))) * 130)} 72% 48%)`;
...
export function FinalThirdPitchView({ data }: { data: FinalThirdShotMapData }) {
  ...
  return <figure className="min-w-0 overflow-hidden rounded-lg border border-white/10 bg-[#07130f]"><svg data-final-third-pitch data-front2-crop="depth5-depth6" viewBox={viewBox} className="block h-auto w-full" role="img" aria-label="Shared Spatial Pitch plan, rotated vertically and cropped to Depths 5 and 6. Attack moves upward and Lane 1 is screen-right.">
    ...
    {data.zones.map((zone) => { const center = tileCenter(zone), unavailable = zone.shotsTotal === null; const innerRadius = unavailable ? 0 : Math.min(OUTER_RADIUS - 8, 8 + Math.sqrt(zone.shotsTotal ?? 0) * 9); return <g key={zone.zoneId} data-zone-id={zone.zoneId} data-zone-state={zone.state} aria-label={`${zone.zoneId}: ${unavailable ? "unavailable" : `${zone.goals} goals from ${zone.shotsTotal} shots, ${displayRate(zone.conversionRatePct)} conversion`}`}>
      ...
      <text x={center.x} y={center.y - 8} textAnchor="middle" fill="white" fontSize="13" fontWeight="800">{zone.goals === null ? "Unavailable" : `${zone.goals}/${zone.shotsTotal}`}</text><text x={center.x} y={center.y + 10} textAnchor="middle" fill="#f4f4f5" fontSize="11">{displayRate(zone.conversionRatePct)}</text>
    </g>; })}
  </svg><figcaption className="px-3 py-2 text-xs text-zinc-300">The same plan pitch geometry is SVG-transformed and front-two-depth cropped. All taxonomy hexes have the same outer footprint; inner fill alone presents server shot volume and quality.</figcaption></figure>;
}
