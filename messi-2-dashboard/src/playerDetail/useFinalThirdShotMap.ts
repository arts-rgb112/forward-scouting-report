...
export type FinalThirdShotMapState = (Keyed & { kind: "disabled" }) | (Keyed & { kind: "loading" }) | (Keyed & { kind: "error" }) | (Keyed & { kind: "unavailable"; data: FinalThirdShotMapEnvelope }) | (Keyed & { kind: "partial"; data: FinalThirdShotMapEnvelope }) | (Keyed & { kind: "ready"; data: FinalThirdShotMapEnvelope }) | (Keyed & { kind: "observed-zero"; data: FinalThirdShotMapEnvelope });
export function useFinalThirdShotMap(config: MessiApiConfig | undefined, playerId: number, context: DatasetRouteState) {
  const [retryEpoch, setRetryEpoch] = useState(0), generation = useRef(0); const enabled = finalThirdShotMapEnabled(); const resourceKey = finalThirdShotMapResourceKey(playerId, context); const [state, setState] = useState<FinalThirdShotMapState>({ kind: enabled ? "loading" : "disabled", key: resourceKey });
  useEffect(() => {
