...
import { fetchFinalThirdShotMapV3, finalThirdShotMapV3ResourceKey } from "../api/finalThirdShotMapV3Api";
import { finalThirdShotMapEnabled, finalThirdShotMapV2Enabled, finalThirdShotMapV3Enabled } from "../api/finalThirdShotMapFeatureGate";
...
import type { FinalThirdShotMapV3Envelope } from "../api/finalThirdShotMapV3Contracts";
import type { MessiApiConfig } from "../api/env";
import type { DatasetRouteState } from "../dashboard/types";

type Keyed = { key: string };
export type FinalThirdShotMapEnvelopeResponse = FinalThirdShotMapEnvelope | FinalThirdShotMapV2Envelope | FinalThirdShotMapV3Envelope;
...
export function useFinalThirdShotMap(config: MessiApiConfig | undefined, playerId: number, context: DatasetRouteState) {
  const [retryEpoch, setRetryEpoch] = useState(0), generation = useRef(0); const enabled = finalThirdShotMapEnabled(); const v2Enabled = finalThirdShotMapV2Enabled(); const v3Enabled = finalThirdShotMapV3Enabled(); const resourceKey = v3Enabled ? finalThirdShotMapV3ResourceKey(playerId, context) : v2Enabled ? finalThirdShotMapV2ResourceKey(playerId, context) : finalThirdShotMapResourceKey(playerId, context); const [state, setState] = useState<FinalThirdShotMapState>({ kind: enabled ? "loading" : "disabled", key: resourceKey });
  useEffect(() => {
    ...
    const request = v3Enabled ? fetchFinalThirdShotMapV3(config, playerId, context, controller.signal) : v2Enabled ? fetchFinalThirdShotMapV2(config, playerId, context, controller.signal) : fetchFinalThirdShotMap(config, playerId, context, controller.signal);
  ...
  }, [config, context.competition, context.mode, context.scope, context.season, enabled, playerId, retryEpoch, resourceKey, v2Enabled, v3Enabled]);
