...
export function PitchSelectionCard(props: PitchSelectionCardProps) {
  ...
  return <section aria-label="선택한 피치 정보" data-pitch-selection-card={kind}
    className="overflow-hidden rounded-2xl border border-[#4a4e52] bg-[#232628]/95 text-[#f1f0ec] shadow-[0_18px_40px_rgba(0,0,0,.28)] backdrop-blur-md">
    <header className="flex items-start justify-between gap-3 px-4 pt-4">
      <div><p className="text-xs tracking-wide text-[#aeb2b3]">{sourceLabel}</p>
    ...
    <div className="mx-4 mt-3 flex items-end justify-between border-b border-white/10 pb-3">
      <div><p className="text-xs text-[#aeb2b3]">슈팅 퀄리티</p>
      ...
      <div className="text-right text-xs text-[#aeb2b3]">xGOT − xG
    ...
    <div className="px-4 pb-4">
      <dl className="grid grid-cols-3 gap-3 border-t border-white/10 pt-3">
        {[["슛", shots ?? "—"], ["득점", goals ?? "—"], ["xG", metric(xg)]].map(([label, value]) =>
          <div key={label}><dt className="text-xs text-[#aeb2b3]">{label}</dt><dd className="mt-1 font-mono text-lg font-semibold">{value}</dd></div>)}
      </dl>
      {kind !== "shot" && <dl className="mt-3 space-y-2 text-xs">
        <div className="flex justify-between"><dt className="text-[#aeb2b3]">활동 비중</dt><dd className="font-mono">{percent(activitySharePct)}</dd></div>
        <div className="flex justify-between"><dt className="text-[#aeb2b3]">슈팅 비중</dt><dd className="font-mono">{percent(shootingSharePct)}</dd></div>
      </dl>}
      {controls && <div className="mt-3 border-t border-white/10 pt-3">{controls}</div>}
      {details && <details className="mt-3 text-xs text-[#aeb2b3]"><summary className="cursor-pointer">기록·표본 상세</summary><div className="mt-2 space-y-1">{details}</div></details>}
    </div>
  </section>;
}
