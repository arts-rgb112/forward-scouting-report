...
describe("native player detail panels", () => {
  ...
  it("puts one interactive arena scene before the tactical note and lazy-mounts the legacy pitch workspace", () => {
    ...
    expect(outer).toHaveClass("min-w-0"); expect(arena).toBeInTheDocument(); expect(arena?.querySelectorAll("canvas")).toHaveLength(1); expect(arena?.querySelector('[data-pitch-presentation="arena"]')).toBeInTheDocument(); expect(within(arena as HTMLElement).getAllByRole("img", { name: "M.E.S.S.I. 카테고리 레이더 데이터 없음" })).toHaveLength(1);
    expect(arena).toHaveAttribute("data-view", "overview");
    fireEvent.click(within(arena as HTMLElement).getByRole("button", { name: "상세 분석" }));
    expect(arena).toHaveAttribute("data-view", "detail");
    expect(within(arena as HTMLElement).getAllByRole("img", { name: "M.E.S.S.I. 카테고리 레이더 데이터 없음" })).toHaveLength(2);
