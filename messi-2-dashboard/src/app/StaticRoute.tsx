...
function LegacyAnalysisSummary({ analysis, quality }: { analysis?: PlayerAnalysis; quality?: QualityDisplay }) {
  ...
  return <section className="mt-6 rounded border border-white/10 bg-black/20 p-4" aria-label="Server analysis"><h2 className="font-bold">Server analysis</h2><p className="mt-2 flex items-center text-sm text-zinc-300">Score {analysis.score.value} · cohort {analysis.score.population}<DataQualityBadge quality={quality} /></p><div className="mt-4 grid gap-4 md:grid-cols-2"><MetricTable title="Volume profile" axes={analysis.volumeRadar.axes} quality={quality} /><MetricTable title="Ratio profile" axes={analysis.ratioRadar.axes} quality={quality} /></div><p className="mt-4 text-base text-zinc-400">Spatial data: {analysis.spatial.available ? `${analysis.spatial.heatmapPointCount} server-provided points` : "unavailable for this context"}.</p>{metrics.length > 0 && <dl className="mt-3 grid grid-cols-2 gap-2 text-base sm:grid-cols-4">{metrics.map(([key, value]) => <div key={key} className="rounded bg-white/[.03] p-2"><dt className="truncate text-zinc-500">{key}</dt><dd className="font-bold text-zinc-200">{String(value)}</dd></div>)}</dl>}</section>;
}
  ...
  return <div>
    ...
    <table className="mt-2 w-full text-left text-base">
      ...
      <tbody>{axes.map((axis) => {
        ...
        return <tr key={axis.id} className="border-t border-white/10"><th className="py-1 font-medium text-zinc-300">{displayRadarAxisLabel(axis)}</th><td className="py-1 text-right text-lime-300">{axis.score}{imputed && <span title={axisQualityCopy(quality!)} className="ml-1 type-caption text-amber-100">대체값</span>}</td><td className="py-1 text-right text-zinc-400">{displayPercent(axis.percentile)}</td><td className="py-1 text-right text-zinc-400">{displayAxisRank(axis)}</td><td className="py-1 text-right text-zinc-400">{axis.population || "—"}</td></tr>;
      })}</tbody>
    </table>
  </div>;
}
...
function ShotmapPitchOverlay({ analysis }: { analysis: PlayerAnalysis }) {
  ...
  return <section aria-label="Shotmap and activity heatmap" className="mt-4 rounded-lg border border-white/10 bg-zinc-950/60 p-3">
    <div className="flex flex-wrap items-baseline justify-between gap-x-3 gap-y-1"><h3 className="text-sm font-bold text-zinc-200">Spatial shotmap</h3><span className="text-base text-zinc-400">{shotState}</span></div>
    <figure className="mt-2" aria-describedby="shotmap-caption">
      ...
      <figcaption id="shotmap-caption" className="mt-2 text-base text-zinc-500">활동 히트맵은 슈팅 데이터가 아닙니다. <span className="text-lime-200">◆ 골</span> · <span className="text-sky-200">● 유효슈팅</span> · <span className="text-rose-200">× 빗나감</span> · <span className="text-amber-200">■ 블록</span></figcaption>
    </figure>
  </section>;
}
...
function TacticalQuadrantChart({ quadrant }: { quadrant?: TacticalQuadrant }) {
  ...
  return <section aria-label="Tactical quadrant" className="mt-4 rounded-lg border border-white/10 bg-zinc-950/60 p-3">
    <div className="flex flex-wrap items-baseline justify-between gap-x-3 gap-y-1"><h3 className="text-sm font-bold text-zinc-200">Tactical quadrant</h3><span className="text-base text-zinc-500">{quadrant.cohortPopulation} players · median split</span></div>
    <svg className="mt-2 w-full" viewBox={`0 0 ${width} ${height}`} role="img" aria-label={`${selected.playerName} tactical quadrant: net progression per 90 against in-box xGOT minus xG`}>
      ...
      <text x={left + 5} y={top + 23} className="fill-sky-200 type-caption">포처</text><text x={width - right - 5} y={top + 23} textAnchor="end" className="fill-lime-200 type-caption">컴플리트 포워드</text><text x={left + 5} y={height - bottom - 10} className="fill-rose-200 type-caption">컴플리트 낙제점</text><text x={width - right - 5} y={height - bottom - 10} textAnchor="end" className="fill-violet-200 type-caption">펄스 나인</text>
      ...
      <text x={left} y={height - 7} className="fill-zinc-500 type-caption">Lower progression</text><text x={width - right} y={height - 7} textAnchor="end" className="fill-zinc-500 type-caption">Higher progression</text><text x={left + 3} y={top + 9} className="fill-zinc-500 type-caption">Higher in-box finish</text><text x={left + 3} y={height - bottom - 5} className="fill-zinc-500 type-caption">Lower in-box finish</text>
    </svg>
    <p className="mt-1 text-base text-zinc-400"><span className="font-medium text-lime-300">{selected.playerName}</span>: {selected.netProgressionPer90.toFixed(2)} net progression/90 · {selected.inBoxXgotMinusXg.toFixed(2)} in-box xGOT − xG</p><p className="sr-only">Quadrants: upper left poacher, upper right complete forward, lower left complete failing point, lower right false nine.</p>
  </section>;
}
...
function AnalysisSummary({ analysis, quality, quadrant }: { analysis?: PlayerAnalysis; quality?: QualityDisplay; quadrant?: TacticalQuadrant }) {
  ...
  return <section className="rounded-xl border border-white/10 bg-white/5 p-4"><div className="flex items-start justify-between gap-3"><div><h2 className="text-lg font-black tracking-tight text-white">Server analysis</h2><p className="text-base text-zinc-400">Score {analysis.score.value} · rank {scoreRank} / cohort {analysis.score.population}<DataQualityBadge quality={quality} /></p></div><span className="rounded bg-lime-300/10 px-2 py-1 text-base font-bold text-lime-200">{analysis.score.archetype}</span></div><div className="mt-4 grid gap-4 md:grid-cols-2"><DetailMetricTable title="Volume radar" axes={analysis.volumeRadar.axes} quality={quality} /><DetailMetricTable title="Ratio radar" axes={analysis.ratioRadar.axes} quality={quality} /></div><p className="mt-4 text-base text-zinc-500">Spatial data: {analysis.spatial.available ? `${analysis.spatial.heatmapPointCount} server-provided points` : "not available"}</p><PositionalGrid analysis={analysis} /><ShotmapPitchOverlay analysis={analysis} /><TacticalQuadrantChart quadrant={quadrant} /><div className="mt-3 grid grid-cols-2 gap-2 text-base text-zinc-400">{Object.entries(analysis.rawMetrics).map(([key, value]) => <div key={key} className="flex justify-between gap-2 border-b border-white/5 py-1"><span>{key}</span><span className="text-zinc-200">{value ?? "—"}</span></div>)}</div></section>;
}
...
export function DuelPressCompanionPanel({ id, dataset, config }: { id: number; dataset: DatasetRouteState; config?: MessiApiConfig }) {
  ...
  return <section aria-label="Duel and pressing companion" className="mt-4 rounded-xl border border-white/10 bg-[#101415] p-4"><h2 className="text-sm font-black">Duel / Press companion</h2><p className="mt-1 text-base text-zinc-400">Server-provided duel and pressing taxonomy for this exact context.</p><div className="mt-4"><DuelPressDetailMetrics player={player}/></div></section>;
}
...
function CompareRoute({ config }: { config?: MessiApiConfig }) {
  ...
  return <Page><ContextBadge dataset={dataset} /><FocusTitle>Player comparison</FocusTitle>{!comparison ? <div aria-busy="true" className="mt-5 h-28 animate-pulse rounded bg-white/10" /> : <div className="mt-5 grid gap-3 sm:grid-cols-2">{comparison.players.map(({ player, analysis }) => <section key={player.id} className="rounded border border-white/10 bg-black/20 p-4"><h2 className="font-bold">{player.name}</h2><p className="text-base text-zinc-500">{player.club.name} · {player.position}</p><p className="mt-2 text-sm text-lime-300">M.E.S.S.I. {player.score}</p><AnalysisSummary analysis={analysis} /></section>)}</div>}<div className="mt-6"><BackLink dataset={dataset} /></div></Page>;
}
...
function ManualSideFields({ label, side, onChange }: { label: string; side: ManualSide; onChange(next: ManualSide): void }) {
  ...
  return <fieldset className="rounded-lg border border-white/10 bg-black/20 p-4"><legend className="px-1 font-black">{label}</legend><div className="grid gap-3 sm:grid-cols-2"><label className="text-base">FotMob player ID<input aria-label={`${label} FotMob player ID`} inputMode="numeric" value={side.playerId || ""} onChange={(event) => patch({ playerId: Number(event.target.value) })} className="mt-1 min-h-11 w-full rounded border border-white/10 bg-[#080b0c] px-2" /></label><label className="text-base">Taxonomy<select aria-label={`${label} taxonomy`} value={side.taxonomy} onChange={(event) => patch({ taxonomy: event.target.value as ManualSide["taxonomy"] })} className="mt-1 min-h-11 w-full rounded border border-white/10 bg-[#080b0c] px-2"><option value="duel-press-v1">Duel / Press v1</option><option value="legacy-v1">Legacy v1</option></select></label><label className="text-base">Season<input aria-label={`${label} season`} value={context.season} onChange={(event) => patch({ context: { ...context, season: event.target.value } as ManualSide["context"] })} className="mt-1 min-h-11 w-full rounded border border-white/10 bg-[#080b0c] px-2" /></label><label className="text-base">Mode<select aria-label={`${label} mode`} value={context.mode} onChange={(event) => patch({ context: event.target.value === "europe" ? { season: context.season, mode: "europe", scope: null, competition: "all" } : { season: context.season, mode: "league", scope: 8, competition: "all" } })} className="mt-1 min-h-11 w-full rounded border border-white/10 bg-[#080b0c] px-2"><option value="league">League</option><option value="europe">Europe</option></select></label>{context.mode === "league" ? <label className="text-base">League scope<select aria-label={`${label} league scope`} value={context.scope} onChange={(event) => patch({ context: { ...context, scope: Number(event.target.value) as 3 | 5 | 7 | 8 } })} className="mt-1 min-h-11 w-full rounded border border-white/10 bg-[#080b0c] px-2">{[3, 5, 7, 8].map((scope) => <option key={scope} value={scope}>{scope} leagues</option>)}</select></label> : <label className="text-base">Europe competition<select aria-label={`${label} Europe competition`} value={context.competition} onChange={(event) => patch({ context: { ...context, competition: event.target.value as "all" | "ucl" | "uel" | "uecl" } })} className="mt-1 min-h-11 w-full rounded border border-white/10 bg-[#080b0c] px-2">{["all", "ucl", "uel", "uecl"].map((competition) => <option key={competition} value={competition}>{competition.toUpperCase()}</option>)}</select></label>}</div></fieldset>;
}
...
function ReadoutValue({ item }: { item: ContextualReadoutItem }) {
  ...
  return <div className="rounded bg-white/[.03] p-2"><dt className="text-zinc-400">{item.label}</dt><dd className="mt-1 font-bold text-zinc-100">{value} <span className="type-caption font-normal text-zinc-400">({item.state.replaceAll("_", " ")})</span></dd><p className="mt-1 type-caption text-zinc-500">{comparisonText}. Source: {item.source.replaceAll("_", " ")}.</p></div>;
}

function DuelPressReadouts({ readout }: { readout: ContextualReadout }) {
  return <section className="mt-3 rounded border border-white/10 p-3" aria-label="Duel and press readouts"><h3 className="text-sm font-bold">Duel / press readouts</h3><p className="mt-1 text-base text-zinc-400">Server-authoritative values for this exact player and context.</p><div className="mt-3 space-y-3">{readout.categories.map((category) => <section key={category.id} className="rounded border border-white/10 p-3"><h4 className="font-semibold">{category.id === "combinedDuel" ? "Combined duel (ground + aerial)" : category.label} {category.score === null ? "— unavailable" : `— score ${category.score}`} <span className="type-caption font-normal text-zinc-400">({category.scoreState})</span></h4>{category.imputedComponents?.length ? <p className="mt-1 text-base text-amber-200">Imputed components: {category.imputedComponents.join(", ")}</p> : null}<dl className="mt-2 grid gap-2 sm:grid-cols-2">{category.readouts.map((item) => <ReadoutValue key={item.id} item={item} />)}</dl></section>)}</div><section className="mt-3 rounded border border-white/10 p-3"><h4 className="font-semibold">Context indicators</h4><p className="mt-1 text-base text-zinc-400">Two server-provided neutral indicators; neither changes the category score.</p><dl className="mt-2 grid gap-2 sm:grid-cols-2">{readout.contextIndicators.map((item) => <ReadoutValue key={item.id} item={item} />)}</dl></section></section>;
}

function AuthoritativeDetail({ side }: { side: ContextualCompareResponse["left"] }) {
  if (side.componentAvailability.detail !== "available" || side.detail === null || side.detail.analysis === undefined) return <section className="mt-3 rounded border border-white/10 p-3"><h3 className="text-sm font-bold">Authoritative detail</h3><p className="mt-2 text-base text-zinc-400">Unavailable for this exact context; no substitute is displayed.</p></section>;
  const metrics = Object.entries(side.detail.analysis.rawMetrics).filter(([, value]) => value !== null).slice(0, 6);
  return <section className="mt-3 rounded border border-white/10 p-3"><h3 className="text-sm font-bold">Authoritative detail</h3><p className="mt-1 text-base text-zinc-400">Server score {side.detail.analysis.score.value}; cohort {side.detail.analysis.score.population}.</p>{metrics.length > 0 && <dl className="mt-2 grid grid-cols-2 gap-2 text-base">{metrics.map(([name, value]) => <div key={name} className="rounded bg-white/[.03] p-2"><dt className="text-zinc-500">{name}</dt><dd className="font-bold">{value}</dd></div>)}</dl>}</section>;
}

function ContextualResolvedSide({ side }: { side: ContextualCompareResponse["left"] }) {
  const unavailable = (name: string) => <section className="mt-3 rounded border border-white/10 p-3"><h3 className="text-sm font-bold">{name}</h3><p className="mt-2 text-base text-zinc-400">Unavailable for this exact context; no domestic or inferred substitute is displayed.</p></section>;
  return <section className="rounded border border-white/10 bg-black/20 p-4"><h2 className="font-bold">{side.summary!.name}</h2><p className="text-base text-zinc-400">{side.context.season} / {side.context.mode} / {side.summary!.club.name}</p><p className="mt-2 text-lime-300">M.E.S.S.I. {side.summary!.score}</p>{side.taxonomy === "duel-press-v1" && side.duelPressPlayer && <section className="mt-3 rounded border border-white/10 p-3"><h3 className="text-sm font-bold">Duel / press category scores</h3><dl className="mt-2 grid grid-cols-2 gap-2 text-base">{Object.entries(side.duelPressPlayer.stats).map(([name, value]) => <div key={name} className="rounded bg-white/[.03] p-2"><dt className="text-zinc-500">{name}</dt><dd className="font-bold">{value}</dd></div>)}</dl></section>}{side.taxonomy === "duel-press-v1" && side.duelPressDetailReadout ? <DuelPressReadouts readout={side.duelPressDetailReadout} /> : null}<AuthoritativeDetail side={side} />{side.componentAvailability.dataQuality === "available" && side.dataQuality ? <section className="mt-3 rounded border border-white/10 p-3"><h3 className="text-sm font-bold">Data quality</h3><p className="mt-1 text-base">Observed weight {side.dataQuality.observedWeightPct}% / {side.dataQuality.imputedMetrics.length ? `imputed: ${side.dataQuality.imputedMetrics.join(", ")}` : "fully observed"}</p></section> : unavailable("Data quality")}{side.componentAvailability.tacticalQuadrant === "available" ? <section className="mt-3 rounded border border-white/10 p-3"><h3 className="text-sm font-bold">Tactical quadrant</h3><p className="mt-1 text-base text-zinc-400">Server-provided for this exact context.</p></section> : unavailable("Tactical quadrant")}</section>;
}
