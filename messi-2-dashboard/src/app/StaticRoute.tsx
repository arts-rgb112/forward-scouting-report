...
function AuthoritativeSide({ side, loading }: { side: import("../api/contextualCompareContracts").ContextualCompareResponse["left"] | undefined; loading: boolean }) {
  ...
  const component = (name: "detail" | "dataQuality" | "tacticalQuadrant") => side.componentAvailability[name] === "available" ? <section className="mt-3 rounded border border-white/10 p-3"><h3 className="text-sm font-bold">{name}</h3>{name === "dataQuality" ? <p className="mt-2 text-xs">Observed weight {side.dataQuality!.observedWeightPct}% · {(side.dataQuality!.imputedMetrics?.length ?? 0) ? `imputed: ${side.dataQuality!.imputedMetrics.join(", ")}` : "fully observed"}</p> : name === "detail" ? <p className="mt-2 text-xs">Server detail available for this exact context.</p> : <p className="mt-2 text-xs">Server tactical quadrant available for this exact context.</p>}</section> : <section className="mt-3 rounded border border-white/10 p-3"><h3 className="text-sm font-bold">{name}</h3><p className="mt-2 text-xs text-zinc-400">Unavailable for this exact context; no substitute is displayed.</p></section>;
  return <section className="rounded border border-white/10 bg-black/20 p-4"><h2 className="font-bold">{side.summary!.name}</h2><p className="text-xs text-zinc-400">{side.context.season} · {side.context.mode} · {side.summary!.club.name}</p><p className="mt-2 text-lime-300">M.E.S.S.I. {side.summary!.score}</p>{component("detail")}{component("dataQuality")}{component("tacticalQuadrant")}{side.duelPressDetailReadout && <section className="mt-3 rounded border border-white/10 p-3"><h3 className="text-sm font-bold">Duel / press readouts</h3><pre className="mt-2 max-h-64 overflow-auto text-[10px]">{JSON.stringify(side.duelPressDetailReadout, null, 2)}</pre></section>}</section>;
}
function ContextualCompareRoute({ config }: { config?: MessiApiConfig }) {
  const canonical = parseDuelPressCompare(window.location.search); const recovered = idsFromUrl();
  const [left, setLeft] = useState<ManualSide>(() => canonical?.left ?? manualSide(recovered?.[0] ?? 0, config)); const [right, setRight] = useState<ManualSide>(() => canonical?.right ?? manualSide(recovered?.[1] ?? 0, config));
  const request = useMemo(() => left.playerId > 0 && right.playerId > 0 ? { comparisonVersion: "contextual-compare-v1" as const, left: { player: { idNamespace: "fotmob" as const, playerId: left.playerId }, taxonomy: left.taxonomy, context: left.context }, right: { player: { idNamespace: "fotmob" as const, playerId: right.playerId }, taxonomy: right.taxonomy, context: right.context } } : null, [left, right]);
  const resource = useContextualCompare(config, request); const href = request ? duelPressCompareHref(left, right) : null;
  return <Page><FocusTitle>Player comparison</FocusTitle><p className="mt-2 text-sm text-zinc-400">Select each player and context independently. Requests use only the server’s contextual comparison contract.</p><div className="mt-5 grid gap-3 lg:grid-cols-2"><ManualSideFields label="Left player" side={left} onChange={setLeft} /><ManualSideFields label="Right player" side={right} onChange={setRight} /></div>{href ? <a href={href} className="mt-4 inline-flex min-h-11 items-center rounded bg-lime-300 px-4 text-sm font-black text-black">Copy exact comparison URL</a> : <p role="alert" className="mt-4 text-sm text-amber-200">Enter two valid FotMob player IDs to request an exact comparison.</p>}{resource.state === "error" && <div role="alert" className="mt-4 rounded border border-rose-300/30 p-3"><p>{resource.error}</p><button type="button" onClick={resource.retry} className="mt-2 min-h-11 rounded border border-lime-300/40 px-3 text-lime-300">Retry comparison</button></div>}<div className="mt-5 grid gap-3 lg:grid-cols-2"><AuthoritativeSide side={resource.value?.left} loading={resource.state === "loading"} /><AuthoritativeSide side={resource.value?.right} loading={resource.state === "loading"} /></div><div className="mt-6"><a href="/" className="text-lime-300 hover:underline">Leaderboard</a></div></Page>;
}
