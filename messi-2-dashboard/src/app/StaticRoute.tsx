...
function AuthoritativeSide({ side, loading }: { side: import("../api/contextualCompareContracts").ContextualCompareResponse["left"] | undefined; loading: boolean }) {
  ...
  if (side.status === "unavailable") return <section className="rounded border border-amber-300/30 bg-amber-300/10 p-4"><h2 className="font-bold">Exact context unavailable</h2><p className="mt-2 text-sm">No server-authoritative data is available. Nothing is derived from a domestic context.</p></section>;
  const component = (name: "detail" | "dataQuality" | "tacticalQuadrant") => side.componentAvailability[name] === "available" ? <section className="mt-3 rounded border border-white/10 p-3"><h3 className="text-sm font-bold">{name}</h3><pre className="mt-2 max-h-64 overflow-auto text-[10px]">{JSON.stringify(side[name], null, 2)}</pre></section> : <section className="mt-3 rounded border border-white/10 p-3"><h3 className="text-sm font-bold">{name}</h3><p className="mt-2 text-xs text-zinc-400">Unavailable for this exact context; no substitute is displayed.</p></section>;
