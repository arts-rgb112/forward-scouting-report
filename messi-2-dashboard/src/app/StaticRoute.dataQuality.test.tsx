...
describe("native detail semantics", () => {
  ...
  it("renders unavailable, verified-zero, and populated actual shot states with noninteractive legacy layers", async () => {
    ...
    expect(within(pitch).getByRole("img")).toHaveAccessibleName(/1 activity points.*1 shots/); expect(pitch).toHaveTextContent("Goals 1"); expect(pitch.querySelectorAll("svg")).toHaveLength(2); expect(pitch.querySelector("canvas[data-layer=\"legacy-density\"]")).toBeInTheDocument(); expect(pitch.querySelectorAll("[tabindex]")).toHaveLength(0);
  });
});
