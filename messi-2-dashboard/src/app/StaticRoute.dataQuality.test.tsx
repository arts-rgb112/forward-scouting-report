...
describe("native detail semantics", () => {
  ...
  it("renders unavailable, verified-zero, and populated actual shot states with accessible legacy outcome markers", async () => {
    ...
    expect(within(pitch).getByRole("img", { name: /1 activity points.*1 shots/ })).toHaveAccessibleName(/Visible shot outcomes: Goals/); expect(pitch).toHaveTextContent("Goals 1"); expect(pitch.querySelectorAll("svg")).toHaveLength(2); expect(pitch.querySelector("canvas[data-layer=\"legacy-density\"]")).toBeInTheDocument(); expect(pitch.querySelectorAll("[data-shot-index][tabindex=\"0\"]")).toHaveLength(1);
  });
});
