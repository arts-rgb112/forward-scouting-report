...
const APP_NAV_COPY = { leaderboard: "선수 순위", compare: "선수 비교", about: "M.E.S.S.I. 지표" } as const;
...
export function GlobalNavigation({ pathname }: { pathname: string }) {
  ...
  const item = "min-h-11 whitespace-nowrap rounded-md px-2 py-2 text-base font-bold text-zinc-400 hover:text-lime-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-lime-300 sm:px-3";
  ...
  return <header className="border-b border-white/10 bg-[#080b0c]/95">
    ...
    <div className="mx-auto flex max-w-[1580px] flex-wrap items-center justify-between gap-x-3 px-3 sm:px-6 lg:px-8">
      <a href="/" className="min-h-11 py-3 text-base font-black tracking-[.18em] text-lime-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-lime-300">M.E.S.S.I.</a>
      <nav aria-label="주요 탐색" className="flex min-w-0 flex-1 flex-wrap items-center justify-end gap-1">
        <a href="/" aria-current={active === "leaderboard" ? "page" : undefined} className={item}>{APP_NAV_COPY.leaderboard}</a>
        <a href={compareHref} aria-current={active === "compare" ? "page" : undefined} className={item}>{APP_NAV_COPY.compare}</a>
        <a href={aboutHref} aria-current={active === "about" ? "page" : undefined} className={item}>{APP_NAV_COPY.about}</a>
      </nav>
    </div>
  </header>;
}
