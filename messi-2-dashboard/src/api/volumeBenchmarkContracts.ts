...
const axis = (id: typeof volumeBenchmarkAxisIds[number]) => z.object({ id: z.literal(id), label: z.string().min(1), playerScore: score, averageScore: score, playerRawValue: z.number().finite().nullable(), averageRawValue: z.number().finite().nullable(), playerRank: z.number().int().positive(), population: z.number().int().positive(), tier: z.enum(["S", "A", "B", "C", "D"]), imputed: z.boolean() }).strict().superRefine((value, ctx) => { if (value.playerRank > value.population) ctx.addIssue({ code: "custom", path: ["playerRank"], message: "rank must not exceed population" }); });
const contextSchema = z.object({ mode: z.enum(["league", "europe"]), scope: z.union([z.literal(3), z.literal(5), z.literal(7), z.literal(8)]).nullable(), competition: z.enum(["all", "ucl", "uel", "uecl"]).nullable() }).strict();
...
const completeData = z.object({ ...identity, sourceContext: contextSchema, benchmark: benchmarkSchema, available: z.literal(true), reason: z.literal("complete"), axes: z.tuple(volumeBenchmarkAxisIds.map((id) => axis(id)) as [ReturnType<typeof axis>, ReturnType<typeof axis>, ReturnType<typeof axis>, ReturnType<typeof axis>, ReturnType<typeof axis>, ReturnType<typeof axis>]) }).strict();
