...
describe("contextual-compare-v1 strict sides", () => {
  ...
  it("allows the same player/context when taxonomy differs", () => { const value = request(); value.right = structuredClone(value.left); value.right.taxonomy = "legacy-v1"; expect(contextualCompareRequestSchema.parse(value).right.taxonomy).toBe("legacy-v1"); });
  ...
  it("rejects malformed strict nested detail and duel provenance data", () => {
    for (const mutate of [(value: any) => { value.left.duelPressPlayer.extra = true; }, (value: any) => { value.left.detail.analysis.volumeRadar.axes[0].extra = true; }, (value: any) => { value.left.detail.analysis.spatial.shotmapPointCount = 1; value.left.detail.analysis.spatial.shotmapPoints = [{ x: 10, y: 10, outcome: "goal", arbitrary: true }]; }, (value: any) => { [value.left.duelPressDetailReadout.categories[0], value.left.duelPressDetailReadout.categories[1]] = [value.left.duelPressDetailReadout.categories[1], value.left.duelPressDetailReadout.categories[0]]; }, (value: any) => { value.left.duelPressDetailReadout.categories[3].readouts[0].id = "outsideBoxShots"; }, (value: any) => { value.left.duelPressDetailReadout.categories[2].readouts[1].direction = "higher_is_better"; }, (value: any) => { value.left.duelPressDetailReadout.categories[5].readouts[1].source = "server_derived"; }, (value: any) => { value.left.duelPressDetailReadout.contextIndicators.pop(); }, (value: any) => { value.left.duelPressDetailReadout.categories[0].readouts[0].source = "server_derived"; }, (value: any) => { Object.assign(value.left.duelPressDetailReadout.categories[0].readouts[0], { state: "imputed", source: "player_season_total", missingComponents: [] }); }, (value: any) => { value.left.detail.extra = true; }]) { const value = response(); mutate(value); expect(() => parseContextualCompareResponse(value, request())).toThrow(); }
  });
});
