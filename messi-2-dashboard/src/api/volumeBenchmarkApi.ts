...
export function volumeBenchmarkUrl(config: MessiApiConfig, playerId: number, context: DatasetRouteState): URL {
...
export async function fetchVolumeBenchmark(config: MessiApiConfig, playerId: number, context: DatasetRouteState, signal: AbortSignal): Promise<VolumeBenchmark> {
  const url = volumeBenchmarkUrl(config, playerId, context);
  ...
  if (data.playerId !== playerId || data.idNamespace !== "fotmob" || data.season !== context.season || data.sourceContext.mode !== context.mode || data.sourceContext.scope !== expectedScope || data.sourceContext.competition !== expectedCompetition) throw new MessiApiError("schema", "Volume benchmark response identity did not match request");
  return data;
}
