...
export const adaptAnalysis = (analysis: PlayerAnalysis): PlayerAnalysis => ({ ...analysis, score: { ...analysis.score }, volumeRadar: { ...analysis.volumeRadar, axes: analysis.volumeRadar.axes.map((axis) => ({ ...axis })) }, ratioRadar: { ...analysis.ratioRadar, axes: analysis.ratioRadar.axes.map((axis) => ({ ...axis })) }, rawMetrics: { ...analysis.rawMetrics }, spatial: { ...analysis.spatial, heatmapPoints: analysis.spatial.heatmapPoints.map((point) => ({ ...point })), shotmapPoints: analysis.spatial.shotmapPoints.map((point) => ({ ...point, ...(point.trajectory ? { trajectory: { ...point.trajectory } } : {}) })), continuousCore: { ...analysis.spatial.continuousCore }, laneRatios: [...analysis.spatial.laneRatios], depthRatios: [...analysis.spatial.depthRatios], positionalGrid: analysis.spatial.positionalGrid.map((cell) => ({ ...cell })), trueCore: { ...analysis.spatial.trueCore, zoneIds: [...analysis.spatial.trueCore.zoneIds], zones: analysis.spatial.trueCore.zones.map((zone) => ({ ...zone })) } } });
