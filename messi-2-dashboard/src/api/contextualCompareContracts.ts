...
const detail = summary.extend({ analysis: z.object({ score: z.object({ value: score, rank: z.number().int().positive().nullable(), topPercent: score.nullable(), population: z.number().int().nonnegative(), archetype: z.enum(["Type A", "Type B"]) }).strict(), volumeRadar: z.object({ kind: z.literal("volume"), axes: z.array(z.object({ id: z.string().min(1), label: z.string().min(1), score }).passthrough()).length(6) }).strict(), ratioRadar: z.object({ kind: z.literal("ratio"), axes: z.array(z.object({ id: z.string().min(1), label: z.string().min(1), score }).passthrough()).length(6) }).strict(), rawMetrics: z.record(z.string(), z.number().finite().nullable()), spatial: z.object({ available: z.boolean(), heatmapPointCount: z.number().int().nonnegative(), heatmapPoints: z.array(z.object({ x: score, y: score }).strict()), shotmapPointCount: z.number().int().nonnegative(), shotmapPoints: z.array(z.unknown()), shotmapSnapshotAvailable: z.boolean(), trueCore: z.object({ available: z.boolean() }).passthrough(), continuousCore: z.object({ available: z.boolean() }).passthrough() }).passthrough() }).strict() });
const tacticalQuadrant = tacticalQuadrantEnvelopeSchema.shape.data;
const duelPressPlayer = summary.extend({ stats: z.object({ outsideShot: score, boxThreat: score, dangerZone: score, combinedDuel: score, spaceControl: score, forwardPress: score }).strict(), components: z.object({ combinedDuelVolume: score, combinedDuelEfficiency: score, recoveries: score, finalThirdPossessionsWon: score }).strict(), pressingRawMetrics: z.record(z.string(), z.number().finite().nullable()) }).omit({ nation: true });
const comparison = z.object({ state: z.enum(["available", "unavailable", "not_applicable"]), median: z.number().finite().nullable(), rank: z.number().int().positive().nullable(), percentile: z.number().finite().min(0).max(100).nullable(), population: z.number().int().nonnegative() }).strict();
...
const side = z.object({
  ...
  duelPressPlayer: duelPressPlayer.nullable(), duelPressDetailReadout: duelPressDetailReadout.nullable(), summary: summary.nullable(),
