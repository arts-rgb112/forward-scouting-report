...
const analysisSchema = z.object({ score: z.object({ value: score, rank: z.number().int().positive().nullable(), topPercent: score.nullable(), population: z.number().int().nonnegative(), archetype: z.enum(["Type A", "Type B"]) }).strict(), volumeRadar: radarSchema.extend({ kind: z.literal("volume") }), ratioRadar: radarSchema.extend({ kind: z.literal("ratio") }), rawMetrics: rawMetricsSchema, spatial: z.object({ available: z.boolean(), source: z.literal("messi-static-cohort"), heatmapPointCount: z.number().int().nonnegative(), heatmapPoints: z.array(z.object({ x: score, y: score }).strict()), inBoxRatio: score.nullable(), outBoxFinalRatio: score.nullable(), midThirdRatio: score.nullable(), finalThirdRatio: score.nullable(), ccaAreaPct: score.nullable(), laneRatios: z.array(z.number().finite()).max(5), depthRatios: z.array(score).max(6), positionalGrid: z.array(positionalGridCellSchema).max(30), dangerZoneDensity: score.nullable(), deepBoxZoneScore: score.nullable() }).strict() }).strict();
