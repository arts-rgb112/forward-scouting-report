...
const shotTrajectorySchema = z.object({ schemaVersion: z.literal("shotmap-trajectory-v1"), endpointKind: z.enum(["goal_mouth", "blocked"]), endX: score, endY: score, endZMeters: z.number().finite().nonnegative().nullable(), source: z.literal("fotmob") }).strict();
const shotmapPointSchema = z.object({ x: score, y: score, outcome: z.enum(["goal", "on_target", "off_target", "blocked"]), xg: z.number().finite().nonnegative().nullable().optional(), xgot: z.number().finite().nonnegative().nullable().optional(), trajectory: shotTrajectorySchema.nullable().optional() }).strict().superRefine((shot, ctx) => {
  if (!shot.trajectory) return;
  const blocked = shot.outcome === "blocked";
  if (blocked && shot.trajectory.endpointKind !== "blocked") ctx.addIssue({ code: "custom", path: ["trajectory", "endpointKind"], message: "blocked shots require a blocked endpoint" });
  if (blocked && shot.trajectory.endZMeters !== null) ctx.addIssue({ code: "custom", path: ["trajectory", "endZMeters"], message: "blocked endpoints require null endZMeters" });
...
// The detail endpoint is namespaced while leaderboard rows deliberately are not.
