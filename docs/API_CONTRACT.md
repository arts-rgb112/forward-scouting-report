...
- DETAIL-MAIN 시각 재검수 보강(2026-09-09): 작업 폴더 C:/Users/USER/Downloads/files/forward-scouting-report-detail-main-20260909. 15b83fc 기능 검증 통과와 별개로 골망·중거리 잔디·활동층 구분·모바일 카테고리 우선순위가 시각 NO-GO. arena 전용 표현을 재보강하며 원본 이벤트/정규화/CCA/API/production은 변경하지 않음. 사분면은 별도 계약 미완 상태 유지, OPEN_WORK 정본.
...
- DETAIL-MAIN-20260908 공간형첫화면(2026-09-09): 작업 폴더 C:/Users/USER/Downloads/files/forward-scouting-report-detail-main-20260909. 최신레퍼런스에맞춰기존3D+정본HUD를ArenaStage로통합; 기존3API/analysis재사용·한canvas·HUD교대·2D보존. schema/점수/원천불변, 새tracking/통계생성없음. 구현·시각검수진행, 정본OPEN_WORK.md.
...
- 상태: 로컬 수정·정적 검증 완료, 호스트 테스트 대기 — Slack 감시기 폴백 GET 호출 및 Socket Mode message 이벤트 경로 점검 (2026-09-04 KST)
- 작업 폴더: C:/Users/USER/Downloads/files/forward-scouting-report-watcher
- 범위/결과: `messi-2-dashboard/slack_watcher.py` 전체의 Slack Web API 호출을 점검해 GET 메서드 `conversations.list`·`conversations.history`·`conversations.replies` 3곳을 `_json_request`에서 `_query_request`로 교정했다. 남은 `_json_request` 호출은 POST 메서드 `conversations.open`·`chat.postMessage`·`reactions.add`뿐이다. `test_poll_fallback_uses_get_query_helper_for_slack_read_methods`를 추가해 세 GET 메서드가 쿼리 헬퍼만 사용하고 POST 헬퍼를 호출하지 않음을 검증하도록 했으며, 기존 channel/DM 일반 `message` 테스트에는 `seenMessageKeys` 2건 기록 assertion을 보강했다. Socket Mode는 `SlackSocketBridge.listen()`이 명령 필터 전에 모든 envelope를 `process_envelope`로 전달하고 `_message_from_envelope()`가 `message`와 `app_mention`을 모두 허용하므로 코드상 app-mention 전용이 아니다. 테스트는 16→17건이며 사용자 지시와 Python/`py` 실행 파일 부재에 따라 샌드박스에서는 실행하지 않았다. `git diff --check` 통과, mechanical gate 대상 없음. HEAD `c9f0857d18187a2b2176c63d318e7abb0c126d3f`, commit·push·PR·merge·deploy·기능 플래그 변경 없음.
