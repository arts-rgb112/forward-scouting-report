...
- 상태: 커밋·PR·Vercel Preview QA 승인됨 — main 병합·Production 환경변수 변경은 별도 사용자 승인 대기
...
- 다음 단계: 승인된 커밋·PR과 Vercel Preview QA를 진행한다. Preview/Production에서 `VITE_GOAL_MOUTH_BASELINE_ENABLED` 및 `VITE_FINAL_THIRD_SHOT_MAP_V3_ENABLED` 값은 바꾸지 않으며, main 병합은 Preview QA 후 별도 사용자 승인이 필요하다.
